...
void JSConfigManager::setRuntimeHelper(const std::shared_ptr<JSRuntimeHelper> runtimeHelper) {
...
void JSConfigManager::setSharedTansitionConfig(jsi::Runtime &rt, const jsi::Value &config) {
  ...
  sharedTransitionConfig_[key] = static_cast<SharedTransitionType>(animationType);
}
