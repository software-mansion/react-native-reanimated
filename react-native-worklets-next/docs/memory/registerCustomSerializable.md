...
`registerCustomSerializable` lets you register your own pre-serialization and post-deserialization logic. This is necessary for objects with prototypes different than just `Object.prototype` or some other built-in prototypes like `Map` etc. The Worklets library can't handle such objects by default to convert into [Serializables](/docs/memory/serializable) hence you need to register them as **Custom Serializables**. This way you can tell the Worklets library how to transfer your custom data structures between different Runtimes without manually serializing and deserializing them every time.
...
| determine | `(value: object) => value is TValue` | A [worklet function](/docs/tutorials/concurrency-model#worklet-functions) that checks whether a given JavaScript value is of the type handled by this Custom Serializable.                                                                                                                                                        |
| pack      | `(value: TValue) => TPacked`         | A [worklet function](/docs/tutorials/concurrency-model#worklet-functions) that packs the JavaScript value of type `TValue` into a value that can be serialized by default as [Serializable](/docs/memory/serializable). The function must return a [supported type for Serialization](/docs/memory/serializable#supported-types). |
| unpack    | `(value: TPacked) => TValue`         | A [worklet function](/docs/tutorials/concurrency-model#worklet-functions) that unpacks the packed value, after it's been deserialized from it's packed form, back into the JavaScript value of type `TValue`.                                                                                                                     |
...
* To use Custom Serializables which require `new` keyword for instantiation, you need to [disable Worklet Classes](/docs/worklets-plugin/plugin-options#disableworkletclasses) option in Worklets Babel plugin configuration.
* Custom Serializables are global and shared between all [Worklet Runtimes](/docs/concepts/runtimeKinds#worklet-runtime). Once you register a Custom Serializable, it will be available in all Runtimes.
* You can use `registerCustomSerializable` only on the [RN Runtime](/docs/concepts/runtimeKinds#rn-runtime).
