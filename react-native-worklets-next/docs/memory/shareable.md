...
An asynchronous method that schedules a callback on the Host Runtime. The callback modifies the `value` of the Shareable Host. You can either pass a value to `setAsync` or a setter worklet function.

### setSync

A synchronous method that preempts the Host Runtime on the current thread and modifies the `value` of the Shareable Host. You can either pass a value to `setSync` or a setter worklet function.
