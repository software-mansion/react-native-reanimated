...
export async function test(t, { setPortalChild, cleanupPortal }) {
  ...
  const describeWithPermissions = shouldSkipTestsRequiringPermissions
    ? t.xdescribe
    : t.describe;
  ...
  describeWithPermissions('Camera', () => {
    ...
    const mountAndWaitFor = (child, propName = 'onCameraReady') =>
      new Promise((resolve) => {
        const response = originalMountAndWaitFor(
          child,
          propName,
          setPortalChild
        );
    ...
    if (Platform.OS !== 'android') {
      t.describe('Camera.takePictureAsync', () => {
        ...
        t.it(
          'returns proper `exif.Flash % 2 = 0` if the flash is off',
          async () => {
        ...
        if (Platform.OS === 'ios') {
          ...
          t.it(
            'returns proper `exif.Flash % 2 = 1` if the flash is on',
            async () => {
              await mountAndWaitFor(
                <Camera
                  ref={refSetter}
        ...
        t.it(
          'returns `exif.WhiteBalance = 1` if white balance is manually set',
          async () => {
            await mountAndWaitFor(
              <Camera
                style={style}
        ...
        t.it(
          'returns `exif.WhiteBalance = 0` if white balance is set to auto',
          async () => {
            await mountAndWaitFor(
              <Camera
                style={style}
        ...
        if (Platform.OS === 'ios') {
          t.it(
            'returns `exif.LensModel ~= back` if camera type is set to back',
            async () => {
              await mountAndWaitFor(
                <Camera
                  style={style}
          ...
          t.it(
            'returns `exif.LensModel ~= front` if camera type is set to front',
            async () => {
              await mountAndWaitFor(
                <Camera
                  style={style}
          ...
          t.it(
            'returns `exif.DigitalZoom ~= false` if zoom is not set',
            async () => {
              await mountAndWaitFor(<Camera style={style} ref={refSetter} />);
              const picture = await instance.takePictureAsync({ exif: true });
              t.expect(picture).toBeDefined();
          ...
          t.it(
            'returns `exif.DigitalZoom ~= false` if zoom is set to 0',
            async () => {
              await mountAndWaitFor(
                <Camera style={style} ref={refSetter} zoom={0} />
              );
              const picture = await instance.takePictureAsync({ exif: true });
              t.expect(picture).toBeDefined();
              t.expect(picture.exif).toBeDefined();
          ...
          t.it('returns `exif.DigitalZoom > 0` if zoom is set', async () => {
            await mountAndWaitFor(
              <Camera style={style} ref={refSetter} zoom={0.5} />
            );
            const picture = await instance.takePictureAsync({ exif: true });
            t.expect(picture).toBeDefined();
            t.expect(picture.exif).toBeDefined();
          ...
          t.it(
            ...
            async () => {
              await mountAndWaitFor(
                <Camera style={style} ref={refSetter} zoom={1} />
              );
              const picture = await instance.takePictureAsync({ exif: true });
              t.expect(picture).toBeDefined();
              t.expect(picture.exif).toBeDefined();
              t.expect(picture.exif.DigitalZoomRatio).toBeGreaterThan(
                smallerRatio
              );
            }
          );
      ...
      t.describe('can record consecutive clips', () => {
        ...
        t.it('started/stopped automatically', async () => {
          ...
          const recordFor = (duration) =>
            new Promise(async (resolve, reject) => {
              try {
                const response = await instance.recordAsync({
