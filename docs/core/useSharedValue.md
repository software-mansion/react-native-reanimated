...
You can also pass a function as `initialValue`. It will be called once on mount and its return value used as the initial shared value. Useful when the initial value is expensive to compute.
...
* Don't store functions (including worklets) inside shared values. This is not supported and may lead to unexpected behavior.
