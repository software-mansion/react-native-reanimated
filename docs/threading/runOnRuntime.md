...
`runOnRuntime` lets you asynchronously run [workletized](/docs/worklets-plugin/about#autoworkletization) functions on a separate worklet runtime on a separate thread.
...
* The callback passed as the argument will not be [workletized](/docs/worklets-plugin/about#autoworkletization) automatically. You need to add `'worklet';` directive manually to allow the function to be run on the [UI thread](/docs/tutorials/concurrency-model#the-react-native-model).
