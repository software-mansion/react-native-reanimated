...
describe('displaySource', () => {
  it('shows only the imports the displayed code uses when boilerplate is on', () => {
    const source = unindent(`
      import { scheduleOnUI, updateScreen } from '../worklets';
      import { fetchFromBackend } from '../mocks';
      ...
      export function* work() {
        yield fetchFromBackend('/x');
      }

      export default function* main() {
        yield scheduleOnUI(work);
      }
    `);
    const { text, rawToDisplayLine } = displaySource(source, {
      boilerplate: true,
    });
    assert.equal(
      text,
    ...
    assert.equal(rawToDisplayLine[10], 5);
    assert.equal(rawToDisplayLine[14], 8);
    assert.equal(
      displaySource(source).text,
      unindent(`
        function work() {
          fetchFromBackend('/x');
        }
