...
export function scanSource(source: string): Map<string, FnInfo> {
  ...
  lines.forEach((text, index) => {
    ...
    if (current === null) {
      ...
      if (fns.has(current.name)) {
        throw new SnippetError(`duplicate function ${current.name}`, line);
      }
      fns.set(current.name, current);
  ...
  if (current !== null) {
    throw new SnippetError(
      `unterminated function ${(current as FnInfo).name}`,
      (current as FnInfo).headerLine
    );
  }
  return fns;
}
