...
`getRuntimeKind` allows you to get the [kind of the current runtime](/docs/0.10-0.12/fundamentals/runtimeKinds). It's useful in advanced scenarios, where you need to check the runtime kind before executing certain code paths or where you need to have separate implementations per runtime.
