...
The Worklets Babel Plugin transforms your code so that it can run on the [Worklet Runtimes](/docs/0.10-0.12/fundamentals/runtimeKinds#worklet-runtime). It looks for functions marked with a `'worklet';` directive and converts them into serializable objects. We call this process [workletization](/docs/0.10-0.12/fundamentals/glossary#to-workletize).
