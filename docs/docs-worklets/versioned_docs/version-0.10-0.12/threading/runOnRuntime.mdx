...
`runOnRuntime` lets you asynchronously run [workletized](/docs/0.10-0.12/fundamentals/glossary#to-workletize) functions on a separate worklet runtime on a separate thread.
...
A reference to a function you want to execute on the [UI thread](/docs/0.10-0.12/fundamentals/glossary#ui-thread) from the [JavaScript thread](/docs/0.10-0.12/fundamentals/glossary#javascript-thread). Arguments to your function have to be passed to the function returned from `runOnUI` i.e. `runOnUI(myWorklet)(10);`.
...
- The callback passed as the argument will not be [workletized](/docs/0.10-0.12/fundamentals/glossary#to-workletize) automatically. You need to add `'worklet';` directive manually to allow the function to be run on the [UI thread](/docs/0.10-0.12/fundamentals/glossary#ui-thread).
