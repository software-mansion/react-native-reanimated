...
You have to explicitly declare which [Worklet Runtime](/docs/0.10-0.12/fundamentals/runtimeKinds#worklet-runtime) will host the Shareable by passing its **runtimeId**.
...
The **runtimeId** of the [Worklet Runtime](/docs/0.10-0.12/fundamentals/runtimeKinds#worklet-runtime) that will host the Shareable. To host on the UI Runtime, use the exported constant [UIRuntimeId](/docs/threading/uiRuntimeId). To host on a Worker Runtime created with [createWorkletRuntime](/docs/threading/createWorkletRuntime), pass its `runtimeId` property.
...
A [worklet](/docs/0.10-0.12/fundamentals/glossary#worklet) that takes the created [Shareable Host](/docs/memory/shareable#shareable-host) and decorates it with additional properties or methods. You can use it to add custom logic to the Shareable, like adding more APIs, overriding existing ones, or adding side effects to them. It must return the same reference it received.

#### guestDecorator

A [worklet](/docs/0.10-0.12/fundamentals/glossary#worklet) that decorates each [Shareable Guest](/docs/memory/shareable#shareable-guest) created for the Shareable with additional properties or methods. You can use it to add custom logic to the Shareable, like adding more APIs, overriding existing ones, or adding side effects to them. It must return the same reference it received.
