...
`createWorkletRuntime` lets you create a new JS runtime which can be used to run [worklets](/docs/0.9/fundamentals/glossary#worklet) possibly on different threads than [JS](/docs/0.9/fundamentals/glossary#javascript-thread) or [UI thread](/docs/0.9/fundamentals/glossary#ui-thread). Use this function if you need to integrate with Worklet Runtimes in C++.
