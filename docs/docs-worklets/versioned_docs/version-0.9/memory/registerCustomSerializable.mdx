...
| determine | `(value: object) => value is TValue` | A [worklet](/docs/0.9/fundamentals/glossary#worklet) that checks whether a given JavaScript value is of the type handled by this Custom Serializable.                                                                                                                                                        |
| pack      | `(value: TValue) => TPacked`         | A [worklet](/docs/0.9/fundamentals/glossary#worklet) that packs the JavaScript value of type `TValue` into a value that can be serialized by default as [Serializable](/docs/memory/serializable). The function must return a [supported type for Serialization](/docs/memory/serializable#supported-types). |
| unpack    | `(value: TPacked) => TValue`         | A [worklet](/docs/0.9/fundamentals/glossary#worklet) that unpacks the packed value, after it's been deserialized from it's packed form, back into the JavaScript value of type `TValue`.                                                                                                                     |
...
- To use Custom Serializables which require `new` keyword for instantiation, you need to [disable Worklet Classes](/docs/0.9/worklets-babel-plugin/plugin-options#disableworkletclasses) option in Worklets Babel plugin configuration.
- Custom Serializables are global and shared between all [Worklet Runtimes](/docs/0.9/fundamentals/runtimeKinds#worklet-runtime). Once you register a Custom Serializable, it will be available in all Runtimes.
- You can use `registerCustomSerializable` only on the [RN Runtime](/docs/0.9/fundamentals/runtimeKinds#rn-runtime).
