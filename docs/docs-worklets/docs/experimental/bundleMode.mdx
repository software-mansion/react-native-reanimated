...
1. Make sure to reinstall your pods.
1. Alternatively, you can run `pod install` with the environment variable set:

   ```sh
   WORKLETS_BUNDLE_MODE=1 pod install
   ```

### Android

1. **Modify your `gradle.properties`**. In your `android/gradle.properties`, add the following:
