...
**Problem:** This can happen in Expo apps due to eager loading being the default. Worklets needs to set up properties on the global object before imports are evaluated. With eager loading, those properties are missing at evaluation time and the imports crash.
