...
<Tabs groupId="worklets-plugin" queryString>
  ...
  <TabItem value="oxc" label="OXC plugin">
    ```javascript {2-5,13}
    /** @type {import('react-native-worklets/plugin-oxc').PluginOptions} */
    const workletsPluginOptions = {
      bundleMode: true,
      strictGlobal: true, // optional, but recommended
    }
    ...
    module.exports = {
      ...
      plugins: [
        ...
        ['react-native-worklets/plugin-oxc/babel', workletsPluginOptions],
      ],
    };
    ```
  </TabItem>
...
:::info
Starting from Worklets 0.13, you can opt in to use the [OXC plugin](/docs/worklets-plugin/oxc), a faster Rust port of the Worklets Babel plugin built for the Bundle Mode. Its setup differs only in the plugin name in `babel.config.js`.
:::
