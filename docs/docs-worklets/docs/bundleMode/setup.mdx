...
Bundle Mode is the only mode of running worklets, so every native app using `react-native-worklets` has to be configured as follows:

## Configure Metro

**Modify your Metro configuration**. Metro needs to be configured to pre-load Bundle Mode entry points into your JavaScript bundle. In your `metro.config.js` file, apply the config required for Bundle Mode. For example:
...
_This step is technically optional, but seamless bundling doesn't work without it._
...
_This step is technically optional, but Fast Refresh doesn't reach Worklet Runtimes without it._
...
And that's it! You can now happily enjoy worklets! You can learn more about using Bundle Mode [here](/docs/bundleMode/usage).
