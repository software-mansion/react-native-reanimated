...
In React Native ecosystem, new JavaScript runtimes don't come with networking capabilities out of the box. The Worklets library provides a simplified version of `fetch` API that can be used on Worklet Runtimes - however, it might not be sufficient for all use cases.
