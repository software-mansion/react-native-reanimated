...
:::note
This page describes the API to create the new [Shareable](/docs/memory/shareable) type, which is a different concept than the old deprecated one coming from `react-native-reanimated`, available with [makeShareable](/docs/memory/makeShareable) function.
:::

Creates a new [Shareable](/docs/memory/shareable) holding the provided initial value. Returns the created Shareable.

You have explicitly declare which [Worklet Runtime](/docs/fundamentals/runtime-kinds#worklet-runtime) will host the Shareable, by passing its **runtimeId**.
...
The **runtimeId** of the [Worklet Runtime](/docs/fundamentals/runtime-kinds#worklet-runtime) that will host the Shareable. **Currently only hosting Shareable on the UI Runtime is supported.** You can obtain the UI Runtime's id from the exported constant [UIRuntimeId](/docs/threading/uiRuntimeId).

### initial

The initial value of the Shareable. It must be a value that is convertible to a [Serializable](/docs/worklets/memory/serializable).
...
A [worklet](/docs/fundamentals/glossary#worklet) that takes the created [Shareable Host](/docs/memory/shareable#shareable-host) and decorates it with additional properties or methods. You can use it to add custom logic to the Shareable, like more adding APIs, override existing ones or add side effects to them. It must return the same reference it received.

#### guestDecorator

A [worklet](/docs/fundamentals/glossary#worklet) that decorates each [Shareable Guest](/docs/memory/shareable#shareable-guest) created for the Shareable with additional properties or methods. You can use it to add custom logic to the Shareable, like more adding APIs, override existing ones or add side effects to them. It must return the same reference it received.

#### initSynchronously

By default, the Shareable is initialized asynchronously, which means that the [Shareable Host](/docs/memory/shareable#shareable-host) is created upon the time of its first access on the Host Runtime. Setting this flag to `true` forces the Shareable to be initialized synchronously during the call to `createShareable`. It's an escape hatch for situations where your Shareable Host would trigger some side effects during its initialization.
