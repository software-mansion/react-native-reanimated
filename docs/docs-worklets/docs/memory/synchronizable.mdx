...
Available only on Synchronizables created with the [`fixedType` option](/docs/memory/createSynchronizable#config). Sets the value without any locking. It never blocks and doesn't respect the explicit lock, so it can interleave with a transaction. When multiple threads write concurrently, the last write wins. Accepts a plain value only, not a setter function.
