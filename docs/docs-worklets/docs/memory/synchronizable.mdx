...
Available only on Synchronizables created with the [`fixedType` option](/docs/memory/createSynchronizable#config). Sets the value without any locking. It never blocks and doesn't respect the explicit lock, so it can interleave with a transaction. When multiple threads write concurrently, the last write wins.

You can also pass a setter function that receives the previous value and returns the new value. Unlike `setBlocking`, the read and the write are **not atomic** - a concurrent write that happens between them is overwritten.

```tsx
// Set the value to 42
synchronizable.setDirty(42);
// Increment the value by 1 - not atomic, may lose concurrent writes
synchronizable.setDirty((prev) => prev + 1);
```
...
- Synchronizable on a JavaScript Runtime is a wrapper to a reference to the actual Synchronizable living in C++. For non-fixed-type Synchronizables, the value is copied to/from C++ on each access.
