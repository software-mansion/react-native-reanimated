...
It can be used to implement a semi-global state for Runtimes to synchronize with. As it's not tied to any Runtime, reading and writing to it doesn't preempt any Runtimes and could be used to implement communication channels between different Runtimes and/or C++ code.

Synchronizable can be obtained exclusively or non-exclusively. Trying to access it from multiple threads at once could lead to blocking.
...
<figcaption style={{ textAlign: 'center' }}>
  ...
  function executed first. Time axis is not to scale.
</figcaption>
...
Exclusively locks the Synchronizable - other threads can't call `getBlocking`, `setBlocking` or `lock` until the Synchronizable is unlocked. Multiple calls on the same thread do nothing. Trying to call `lock` from another thread while the value is already locked results in blocking.
