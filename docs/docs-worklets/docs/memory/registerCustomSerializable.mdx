...
| determine | `(value: object) => value is TValue` | A [worklet](/docs/tutorials/concurrency-model#worklet-functions) that checks whether a given JavaScript value is of the type handled by this Custom Serializable.                                                                                                                                                        |
| pack      | `(value: TValue) => TPacked`         | A [worklet](/docs/tutorials/concurrency-model#worklet-functions) that packs the JavaScript value of type `TValue` into a value that can be serialized by default as [Serializable](/docs/memory/serializable). The function must return a [supported type for Serialization](/docs/memory/serializable#supported-types). |
| unpack    | `(value: TPacked) => TValue`         | A [worklet](/docs/tutorials/concurrency-model#worklet-functions) that unpacks the packed value, after it's been deserialized from it's packed form, back into the JavaScript value of type `TValue`.                                                                                                                     |
