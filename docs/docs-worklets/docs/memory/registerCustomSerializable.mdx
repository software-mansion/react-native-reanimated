...
`registerCustomSerializable` lets you register your own pre-serialization and post-deserialization logic for object with prototype different than `Object.prototype` that aren't supported by default by [Serializable](/docs/memory/Serializable). Such types are called **Custom Serializables**. This way you can tell Worklets how to transfer your custom data structures between different Runtimes without manually serializing and deserializing them every time.

## Reference

```typescript
const obj = { a: 42 };
Object.setPrototypeOf(obj, global.console); // because why not

registerCustomSerializable({
  name: 'ConsoleLike',
  determine(value: object): value is typeof console {
    'worklet';
    return Object.getPrototypeOf(value) === global.console;
  },
  pack(value: typeof console) {
    'worklet';
    return { a: value.a }; // transfer data
  },
  unpack(value: object) {
    'worklet';
    return Object.create(global.console, value); // recreate object with prototype
  },
});
```
...
scheduleOnUI(() => {
  // This will throw because `obj` had
  // a custom prototype and it couldn't be serialized.
  obj.log('Hello!');
});
```
...
registerCustomSerializable({
  ...
  unpack(value: object) {
    ...
    return Object.create(global.console);
  },
});

scheduleOnUI(() => {
  obj.log('Hello!'); // 'Hello!' printed to console.
});
```
...
- To use Custom Serializables which require `new` keyword for instantiation, you need to disable `workletClasses` feature of `Worklets Babel Plugin`.
