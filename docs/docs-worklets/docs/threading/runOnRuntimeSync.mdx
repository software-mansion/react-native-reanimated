...
`runOnRuntimeSync` lets you run a [workletized](/docs/worklets-babel-plugin/about#autoworkletization) function synchronously on a [Worker Runtime](/docs/concepts/runtimeKinds#worker-runtime). It's blocking - meaning that it can preempt the runtime from a thread that's currently executing it.
