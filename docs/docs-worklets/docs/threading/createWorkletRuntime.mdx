...
When set to `false`, the runtime is created lock-free — no mutex is acquired around JSI operations or synchronous calls, which removes the locking overhead. Use it only when you can guarantee that the runtime is never accessed from more than one thread at a time, e.g. when all work on the runtime is scheduled through its queue.
