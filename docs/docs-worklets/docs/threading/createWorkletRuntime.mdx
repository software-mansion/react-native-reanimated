...
Takes effect only in [Bundle Mode](/docs/bundleMode/); outside of it the networking API is never installed. Always disabled together with the Event Loop, because `whatwg-fetch` resolves its responses through `setTimeout`.
