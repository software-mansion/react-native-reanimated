...
When set to `false`, the runtime takes no locks and is created without the event loop — `enableEventLoop: true` isn't allowed and asynchronous APIs like timers or Promise continuations are unavailable. Custom serializables registered and module updates made after the runtime's creation aren't applied to it. Synchronizing access to the runtime is the caller's responsibility.
