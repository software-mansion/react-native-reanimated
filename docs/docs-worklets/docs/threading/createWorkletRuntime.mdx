...
When set to `false`, per-operation locking is disabled and the runtime is created without the event loop — `enableEventLoop: true` isn't allowed and asynchronous APIs like timers or Promise continuations are unavailable. Work scheduled on the runtime's queue, `registerCustomSerializable`, and module updates during development remain synchronized; synchronous calls such as `runOnRuntimeSync` and direct JSI access must not run concurrently with other work on the runtime.
