...
`runOnJS` lets you asynchronously run non-[workletized](/docs/worklets-babel-plugin/about#autoworkletization) functions that couldn't otherwise run on the [UI thread](/docs/tutorials/concurrency-model#the-react-native-model). This applies to most external libraries as they don't have their functions marked with `"worklet";` directive.
