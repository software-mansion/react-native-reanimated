...
`runOnRuntimeAsync` lets you asynchronously run [worklets](/docs/tutorials/concurrency-model#worklet-functions) on a [Worker Runtime](/docs/concepts/runtimeKinds#worker-runtime).
...
A reference to a function you want to execute on the [Worker Runtime](/docs/concepts/runtimeKinds#worker-runtime). Arguments to your function have to be passed after the worklet i.e. `runOnRuntimeAsync(workletRuntime, setValue, 10);`.
...
- The callback passed as the argument is automatically [workletized](/docs/worklets-babel-plugin/about#autoworkletization) and ready to be run on the [Worker Runtime](/docs/concepts/runtimeKinds#worker-runtime).
