...
### Native side failed to resolve JavaScript code version
...
**Problem:** This happens when Reanimated fails to determine the version of its Java part - most likely because for some reason Java part hasn't rebuilt after an upgrade.
...
### (C++) Mismatch between C++ code version and Java code version

**Problem:** This happens when Reanimated has different versions of its C++ and Java parts.

**Solution:** See [Native side failed to resolve Java code version](#native-side-failed-to-resolve-java-code-version).

### Native side failed to resolve C++ code version
...
### (Java) Mismatch between Java code version and C++ code version
