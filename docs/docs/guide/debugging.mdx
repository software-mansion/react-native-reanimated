...
to the `patches/` folder. *Warnings about mismatched patch versions can
be safely ignored if the patch was successfully applied*.
...
_Debugging relies on source maps that are generated by the babel plugin, so you
...
the reload is performed, the app will crash
...
- The profiler does not work - this is an issue with Hermes and is not related
to reanimated

- The console is sometimes unresponsive

These issues **do not** affect release builds and debug builds, where the debugger connection
