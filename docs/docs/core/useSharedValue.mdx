...
- When the shared value is an object, you cannot alter values individually. Set the value as the complete object.
...
```javascript
  ...
  const pan = Gesture.Pan()
    .onChange((event) => {
      offsets.value.x = event.translationX; // doesn't work
      offsets.value = {x: event.translationX, y: offsets.value.y}; // Correct
    })
```
