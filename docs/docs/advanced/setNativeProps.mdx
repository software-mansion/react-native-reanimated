...
- `setNativeProps` is implemented only on the [UI thread](/docs/fundamentals/glossary#ui-thread). When using `setNativeProps` inside [event handlers](https://react.dev/learn/responding-to-events#adding-event-handlers), it has to be wrapped with the [`runOnUI`](/docs/threading/runOnUI) function.
