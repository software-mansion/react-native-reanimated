...
The handler should be passed to a corresponding `onEvent` prop (e.g. `onScroll` when working with scroll-related handlers). In reality, the composed handler can be hooked into virtually any prop thanks to the way it is detected. Nevertheless, we strongly recommend setting the handler on the prop it belongs to.
