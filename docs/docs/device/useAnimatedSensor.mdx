...
- `ACCELEROMETER` - measures the device acceleration along x, y, and z axes (excluding gravity) in m/s^2.
- `GYROSCOPE` - measures the rate of rotation along x, y, and z axes in radians per second.
- `GRAVITY` - measures acceleration due to gravity along x, y, and z axes in m/s^2.
...
| ---------------------------- | ------------------- | ------------------------ | --------------------------------------------------------------------------------------- |
| interval                     | `number｜"auto"`    | `"auto"`                 | Time in milliseconds between sensor readings. `"auto"` matches the device refresh rate. |
...
| unregister  | `() => void`                          | Allows to stop listening to sensor updates                                                                                                                                                                                                      |
...
Depending on the sensor type the measurement is a number in radians per second, m/s^2 or microtesla (μT).
