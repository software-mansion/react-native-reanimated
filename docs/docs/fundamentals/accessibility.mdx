...
- `withRepeat`
  - when the numOfReps is infinite or even and the animation is reversed, then the repeated animation does not start and the startValue is returned
  - otherwise, the repeated animation runs once
- `withSequence` exclusively starts animations with reduced motion disabled
...
And here `withTiming` will be executed with no delay:
...
- entering and layout transitions instantaneously reach their endpoints.
- exiting and shared transitions are omitted.

## `useReducedMotion`

This hook returns a boolean value that indicates whether the `Reduced Motion` accessibility setting was enabled when the app started.
