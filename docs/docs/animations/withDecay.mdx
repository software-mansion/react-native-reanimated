...
| clamp <Optional footnote={true} /> | `[number, number]` | []                    | Array of two numbers which restricts animation's range. Animation stops when either bound is reached unless the `rubberBandEffect` option is set to `true`. <br /> \*Required when RubberBandEffect is true |
