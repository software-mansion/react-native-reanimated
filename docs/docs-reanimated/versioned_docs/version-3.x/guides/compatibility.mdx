...
For example, `3.17.x` shows support for React Native `0.79` because Reanimated `3.17.5` added it, even though `3.17.0` didn't have it. If you're on an older patch, check the `compatibility.json` file in the Reanimated package for exact version support.
