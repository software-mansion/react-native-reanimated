...
`useHandler` is a low-level hook. It returns a context object and a value that tells you if the worklet needs to be rebuilt. You can use it to create custom event handler hooks, like [`useScrollViewOffset`](/docs/3.x/scroll/useScrollViewOffset/) or [`useAnimatedScrollHandler`](/docs/3.x/scroll/useAnimatedScrollHandler/).
...
This example can be more easily implemented using [`useScrollViewOffset`](/docs/3.x/scroll/useScrollViewOffset/).
