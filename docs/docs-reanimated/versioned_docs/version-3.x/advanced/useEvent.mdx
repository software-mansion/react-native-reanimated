...
`useEvent` is a low-level hook. It returns event handler that will be called when native event occurs. You can use it to create custom event handler hooks, like [`useScrollViewOffset`](/docs/3.x/scroll/useScrollViewOffset/) or [`useAnimatedScrollHandler`](/docs/3.x/scroll/useAnimatedScrollHandler/).
