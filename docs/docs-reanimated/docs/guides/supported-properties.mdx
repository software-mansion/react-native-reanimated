...
Not all CSS properties are available and animatable in React Native. The following table lists, in alphabetical order, which style properties can be animated on which platform. Every listed property has been supported since Reanimated 4.0.0 unless a cell shows the version that added it.
