...
On iOS and Android, `backgroundImage` gradients are interpolated: colors, color stop positions, an angle direction and the radial size and position animate smoothly between keyframes. Two gradients that cannot be interpolated (for example a linear gradient and a radial gradient, or a keyword direction such as `to top right` and an angle) switch at the midpoint of the animation. Layers are paired by their index in the array, so a layer that exists in only one keyframe has to be the last one; it fades in from or out to transparent.
