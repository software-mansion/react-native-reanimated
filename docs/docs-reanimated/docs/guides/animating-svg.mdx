...
The first two drive the prop with a **shared value**: you animate it imperatively (`withTiming`, `withRepeat`, ...) and the prop follows, all on the UI thread. **CSS** is declarative - you describe the animation and Reanimated runs it. They're separate mechanisms; here's the same pulsing circle each way:

```tsx
// Shared value (imperative): you animate r, the prop follows it
const r = useSharedValue(20);
r.value = withRepeat(withTiming(50), -1, true); // e.g. inside an effect
<AnimatedCircle r={r} cx="50%" cy="50%" />;

// CSS animation (declarative): the keyframes animate r for you
<AnimatedCircle
  cx="50%"
  cy="50%"
  r={20}
  animatedProps={{
    animationName: { from: { r: 20 }, to: { r: 50 } },
