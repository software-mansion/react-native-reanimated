...
A transition only reacts to props changing between renders - updating a [shared value](/docs/fundamentals/glossary#shared-value) (`r.value = ...`) won't trigger one. To animate from a shared value, drive the prop with [`useAnimatedProps`](/docs/core/useAnimatedProps) instead.
