...
Point Jest at Reanimated's resolver in your `jest.config.js` file, so the parts of Reanimated that require a real native module are replaced by their web implementations under Jest:
...
The resolver also enforces the web implementation of [React Native Worklets](https://docs.swmansion.com/react-native-worklets/docs/guides/testing), so no additional Worklets setup is needed.

:::info
If your project already uses a custom resolver, call Reanimated's resolver from it and forward the `(request, options)` arguments instead of replacing it.
:::
