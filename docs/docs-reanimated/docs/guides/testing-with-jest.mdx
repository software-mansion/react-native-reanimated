...
Because of how `react-native-svg` manages its props, it is not possible to test it with Reanimated out of the box.
To enable testing, create a mock file called `react-native-svg.js` inside your project's `__mocks__` folder, and then use it in your test files with:

```js
jest.mock('react-native-svg');
```

Here's an example implementation of the mock:
...
module.exports = { Svg, Circle };
```

:::info
You can add more components (e.g. Rect, Path etc.) using the same pattern as we've used above.
:::
