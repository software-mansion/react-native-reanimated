...
1. Clone [react-native-reanimated](https://github.com/software-mansion/react-native-reanimated/)
1. `cd react-native-reanimated`
1. `yarn`
1. `cd docs/docs-reanimated`
1. `yarn && yarn start`
...
1. `yarn && yarn build-all`
1. `(cd apps/fabric-example/ios && bundle install && bundle exec pod install)` – install Pods for iOS
1. `cd apps/fabric-example && yarn start` – make sure to start metro bundler before building the app in Xcode.
