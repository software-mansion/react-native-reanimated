...
A worklet is a short-running JavaScript function that can be moved and executed across different Javascript Runtimes. Libraries like [Reanimated](https://docs.swmansion.com/react-native-reanimated/) use worklets to calculate view styles and react to events on the UI thread.
...
However, the `'worklet'` directive is still necessary on the Web, because libraries like Reanimated rely on the Worklets Babel plugin to capture dependencies inside worklet functions.
