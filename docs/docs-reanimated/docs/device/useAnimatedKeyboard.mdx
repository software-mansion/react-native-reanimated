...
As of `react-native-reanimated` **4.x** the `useAnimatedKeyboard` hook has been deprecated in favor of the more robust and feature-complete [keyboard](https://kirillzyusko.github.io/react-native-keyboard-controller/) library. This guide walks you through the migration process.
...
### Step 2: Add a compatibility wrapper

To make migration easier, you can drop in a small compatibility layer:
...
Replace your existing imports with the compat layer:

```diff
- import { useAnimatedKeyboard, KeyboardState } from 'react-native-reanimated';
+ import { useAnimatedKeyboard, KeyboardState } from './path-to-compat-layer';
```
