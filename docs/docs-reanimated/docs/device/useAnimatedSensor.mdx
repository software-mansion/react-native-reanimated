...
The playground below mimics the hinge sensor of a foldable device:
...
- `HINGE` - measures the fold angle of a foldable device in radians, from `0` (closed) to `π` (flat), together with a `HingeStatus`.
...
| ----------- | ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| sensor      | `SharedValue<Value3D｜ValueRotation｜ValueHinge>` | A [shared value](/docs/fundamentals/glossary#shared-value) containing the sensor measurements. The values returned by the sensor depend on the type of sensor you choose with \[sensor(/docs/docs/device/useAnimatedSensor#sensortype) argument. |
...
- `angle: number` - the angle between the two halves of the device in radians. `0` means the device is closed, `π` means it is flat (fully open).
...
- Samsung Galaxy Z Fold devices report only 0°, 90° and 180° through the public hinge sensor. Pixel Fold and the Android emulator report a continuous angle. Use `status` when your animation must behave the same on every device.
