...
| isAvailable | `boolean`                             | Indicates whether the device has the sensor, already in the first render. It doesn't reflect a missing permission. It's `false` during server rendering and hydration, after which the component renders again with the value for the device.                                                                              |
