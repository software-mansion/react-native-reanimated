...
| isAvailable | `boolean`                                         | Whether the device has the sensor. It's correct from the first render and doesn't reflect a missing permission. On iOS, the hinge sensor is `false` until its first update, and on a device with a hinge it then becomes `true`. |
...
- The hinge sensor needs a foldable device. On Android it reads `Sensor.TYPE_HINGE_ANGLE`. On iOS it needs iOS 27.1 or newer. On Web it is not supported and `isAvailable` is `false`.
