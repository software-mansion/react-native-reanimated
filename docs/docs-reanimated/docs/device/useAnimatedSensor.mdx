...
| isAvailable | `boolean`                             | Indicates whether sensor is available to use. You won't be able to use a sensor when it's missing from the device or the app doesn't have permission to access it. The value is set from the first render. It is `false` during server rendering.                                                                              |
