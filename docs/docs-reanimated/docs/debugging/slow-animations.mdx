...
On Android, Reanimated adds a custom option to the React Native Dev Menu. When enabled, animations run 10 times slower.
...
- If you need to inspect animations frame by frame, record the screen and open the recording in a media player that step through individual frames (i.e. QuickTime Player with the left and right arrow keys).
