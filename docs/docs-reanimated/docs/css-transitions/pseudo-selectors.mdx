...
Give every animated attribute its resting value as `default` inside the pseudo object. The pseudo object owns the attribute, so a plain prop of the same name is ignored, and without `default` the attribute falls back to its own default, which for `r` means the shape renders at `0`.
