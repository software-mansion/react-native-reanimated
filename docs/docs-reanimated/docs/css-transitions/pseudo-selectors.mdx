...
Give every animated attribute a resting value, either as `default` inside the pseudo object or as the plain prop it replaces - both work, and `default` wins when you write both. With neither, the attribute falls back to its own default, which for `r` means the shape renders at `0`.
...
- `default` is optional. Without it the property rests at the value the pseudo object replaced, such as a `backgroundColor` set by an earlier style in the array, or at its own default if nothing set it. A `default` inside the pseudo object wins over both.
