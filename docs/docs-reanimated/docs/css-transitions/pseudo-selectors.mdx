...
- `default` is optional. Without it the property rests at the value it would have without the pseudo object: a `backgroundColor` set by an earlier style in the array, or the property's own default when nothing sets it. A `default` inside the pseudo object wins over both.
