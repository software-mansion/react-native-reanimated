...
Fires when the transition is interrupted before finishing - for example, when the target changes again or the transition config is removed. On Android and iOS, it also fires when the component unmounts. On web, unmounting the component doesn't fire this callback.
