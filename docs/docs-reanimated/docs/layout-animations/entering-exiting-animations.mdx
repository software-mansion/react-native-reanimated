...
<details>
<summary>Target values</summary>

These are the target values for each exiting animation that can be customized with the `withTargetValues` modifier.
...
| `FlipOutXDown`  | `{ perspective: 500, rotateX: '-90deg', translateY: targetValues.currentHeight }` |
| `FlipOutXUp`    | `{ perspective: 500, rotateX: '90deg', translateY: -targetValues.currentHeight }` |
| `FlipOutYLeft`  | `{ perspective: 500, rotateY: '-90deg', translateX: -targetValues.currentWidth }` |
| `FlipOutYRight` | `{ perspective: 500, rotateY: '90deg', translateX: targetValues.currentWidth }`   |
...
| `LightSpeedOutRight` | `{opacity: 0, translateX: values.windowWidth, skewX: '0deg' }` |
...
| Name          | Config                                                        |
| ------------- | ------------------------------------------------------------- |
| `PinwheelIn`  | `{ opacity: 0, transform: [{ scale: 0 }, {rotate: '5rad' }]}` |
...
| `SlideOutLeft`  | `{ originX: Math.max(values.currentOriginX - values.windowWidth, -values.windowWidth)}`   |
| `SlideOutRight` | `{ originX: Math.max(values.currentOriginX + values.windowWidth, values.windowWidth)}`    |
| `SlideOutUp`    | `{ originY: Math.max(values.currentOriginY - values.windowHeight, -values.windowHeight)}` |
...
| ----------------- | ------------------------------------------------- |
| `ZoomOut`         | `{ scale: 0 }`                                    |
| `ZoomOutDown`     | `{ translateY: values.currentHeight, scale: 0 }`  |
...
| `ZoomOutRotate`   | `{ scale: 1, rotate: '0.3' }`                     |
| `ZoomOutUp`       | `{ translateY: -values.currentHeight, scale: 0 }` |
