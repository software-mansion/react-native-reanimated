...
<details>
  ...
  function useSharedValue<Value>(initialValue: Value | (() => Value)
  ): SharedValue<Value>;
  ```
</details>
...
<AvailableFrom version={'4.4.0'}/> You can also pass a function as `initialValue`. It will be called once on mount and its return value used as the initial shared value. Useful when the initial value is expensive to compute.
