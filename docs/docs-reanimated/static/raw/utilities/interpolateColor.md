...
| useCorrectedHSVInterpolation  | `boolean` | true    | Whether to reduce the number of colors the interpolation has to go through. |

#### Options explanation

* *gamma* - Colors on web / mobile are expressed in sRGB colorspace which is gamma-corrected, that is non-linear.
...
* *useCorrectedHSVInterpolation* - Sometimes (for example when interpolating from yellow to purple) HSV interpolation goes
