...
This hook is a counterpart of [`useAnimatedStyle`](useAnimatedStyle) hook, but works for a non-style view properties.
...
Similarily to [`useAnimatedStyle`](useAnimatedStyle), this hook takes a single worklet (it is not necessary to add `worklet` directive here, because the method will be converted to worklet automatically).
