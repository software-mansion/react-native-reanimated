...
def find_config()
  ...
  if react_native_json == nil
    raise '[Reanimated] Unable to recognize your `react-native` version! Please set environmental variable with `react-native` locations: `export REACT_NATIVE_NODE_MODULES_DIR="<path to react-native>" && pod install.'
  end
...
def assert_no_multiple_instances(react_native_info)
  ...
  if reanimated_instances > 1
    ...
    raise "[Reanimated] Multiple versions of Reanimated were detected (iOS). See https://docs.swmansion.com/react-native-reanimated/docs/guides/Troubleshooting#multiple-version-of-reanimated-were-detected \n\nConflict between: \n" + parsed_location
  end
end
...
def assert_no_reanimated2_with_new_architecture(reanimated_package_json)
  ...
  if fabric_enabled && reanimated_major_version == 2
    raise "[Reanimated] Reanimated 2 does not support Fabric. Please upgrade to Reanimated 3 for the New Architecture support. For details, see https://blog.swmansion.com/announcing-reanimated-3-16167428c5f7."
  end
end
...
def assert_latest_react_native_with_new_architecture(config, reanimated_package_json)
  ...
  if fabric_enabled && reanimated_major_version == 3 && react_native_minor_version < 72
    raise "[Reanimated] Outdated version of `react-native` for New Architecture. Reanimated " + reanimated-version + " supports the New Architecture on React Native 0.72.0+."
  end
end
