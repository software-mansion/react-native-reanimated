...
- Platform seam: `RuntimeBindings` (`Common/cpp/worklets/WorkletRuntime/RuntimeBindings.h`), a struct of `std::function`s built by each platform module. Reanimated equivalent: `Common/cpp/reanimated/Tools/PlatformDepMethodsHolder.h`. A complete example of core plus both backends is `requestAnimationFrame`: `Common/cpp/worklets/AnimationFrameQueue/`, `apple/worklets/apple/AnimationFrameQueue.mm`, Kotlin `runloop/AnimationFrameQueue.kt`.
...
- Reanimated C++: `Common/cpp/reanimated/{CSS,Fabric,LayoutAnimations,NativeModules,RuntimeDecorators,Compat}`. The Fabric component for `REASharedTransitionBoundary` is in `Common/NativeView/`.
