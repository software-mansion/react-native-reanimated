...
**Solution:** See [installation docs](/docs/fundamentals/getting-started#installation) for more information.
...
**Problem:** This happens because Expo apps disable [inlineRequires](https://metrobundler.dev/docs/configuration/#gettransformoptions) by default. This breaks the initialization pipeline for Worklets.
...
**Problem:** This can happen when you try to call a function that is neither Worklet nor a Host Function on a different Runtime than it was defined on.
...
In this example, `callee` cannot be called from a worklet ran on UI Runtime because there is no corresponding UI function for `callee`.
...
Check [this page](/docs/fundamentals/glossary#worklet) to learn more about worklets.
