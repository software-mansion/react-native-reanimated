...
const Child = props => {
  ...
  const toBeTested = ['sharedValues', 'worklets', 'eventWorklets'];
  ...
  (() => {
    ...
    if (toBeTested.includes('eventWorklets')) {
      ...
      for (var i = 0; i < n; ++i) {
        eventWorklets.push(
          useEventWorklet(eventWorkletBody, [
            svs[eStartingIndex],
...
class Wrapper extends React.Component {
  ...
  renderChildren = () => {
    if (
      !this.initialState.obtained ||
      this.state.cleanupPhase !== cleanupPhase.rendering
    ) {
  ...
  render() {
    ...
    return <View>{this.renderChildren()}</View>;
  }
}
