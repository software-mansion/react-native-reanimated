...
function MapperTest2() {
    ...
    const mapper = useMapper(function(input, output) {
        'worklet';
    ...
    }, [{totalX, totalY, parentHeight, parentWidth}, {totalX: mappedX, totalY: mappedY}]);
    mapper.startMapping();

    const mapper1 = useMapper(function(input, output) {
        'worklet';
        output.totalX.set(input.parentWidth.value - input.totalX.value - 40);
        output.totalY.set(input.totalY.value);
    }, [{totalX: mappedX, totalY: mappedY, parentHeight, parentWidth}, {totalX: mappedX1, totalY: mappedY1}]);
    mapper1.startMapping();

    const mapper2 = useMapper(function(input, output) {
        'worklet';
        output.totalY.set(input.parentHeight.value - input.totalY.value - 40);
        output.totalX.set(input.totalX.value);
    }, [{totalX: mappedX, totalY: mappedY, parentHeight, parentWidth}, {totalX: mappedX2, totalY: mappedY2}]);
