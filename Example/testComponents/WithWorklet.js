...
const toggleWorklet = new Worklet(function(x, parentWidth, spring) {
  'worklet';
  console.log('jestem');
...
function WithWorkletScreen() {
  ...
  const style = useAnimatedStyle(
    function(input, accessories) {
      ...
      return {
        ...
        transform: [
          {
            translateX: Reanimated.withWorklet(
              spring.worklet,
              [{}, { toValue: 20 }],
              0
            ),
  ...
  const toggle = useWorklet(toggleWorklet, [
    style.transform[0].translateX,
    parentWidth,
    spring,
  ]);
  ...
  const eventWorklet = useEventWorklet(
    function(x, spring) {
      ...
      if (this.event.state === Reanimated.END) {
        x.set(
          Reanimated.withWorklet(spring.worklet, [
            { velocity: this.event.velocityX },
          ])
        );
      }
    },
