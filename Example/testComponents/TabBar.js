...
const Bar = () => {
  ...
  const opWorklet = useEventWorklet(
    function(input) {
      'worklet';
      ...
      if (index !== input.activeIndex.value && input.newIndex.value === -1) {
        ...
        this.log(
          '[event worklet] index switch: ' +
            input.activeIndex.value +
            ' -> ' +
            index
        );
        const newFollowerDest =
          -input.widthSV.value +
          input.tabWidthSV.value / 2 +
          index * input.tabWidthSV.value;
        input.followerPosition.set(
          Reanimated.withWorkletCopy(input.spring.worklet, [
            {},
            {
        ...
        input.tabsSA[input.activeIndex.value].translateY.set(
          Reanimated.withWorkletCopy(input.spring.worklet, [
            {},
        ...
        input.tabsSA[input.newIndex.value].translateY.set(
          Reanimated.withWorkletCopy(input.spring.worklet, [
            {},
        ...
        input.tabsSA[input.newIndex.value].opacity.set(
          Reanimated.withWorkletCopy(input.spring.worklet, [
            {},
            { toValue: input.values.opacity.min.value },
          ])
        );
        input.tabsSA[input.newIndex.value].opacityReverse.set(
          Reanimated.withWorkletCopy(input.spring.worklet, [
            {},
        ...
        input.tabsSA[input.activeIndex.value].opacity.set(
          Reanimated.withWorkletCopy(input.spring.worklet, [
            {},
            { toValue: input.values.opacity.max.value },
          ])
        );
        input.tabsSA[input.activeIndex.value].opacityReverse.set(
          Reanimated.withWorkletCopy(input.spring.worklet, [
            {},
            { toValue: input.values.opacity.min.value },
          ])
