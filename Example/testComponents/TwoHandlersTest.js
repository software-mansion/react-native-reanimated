...
const panWorkletBody = new Worklet(function(
  prev,
  total,
  velocity,
  cord,
...
const TwoHandlersTest = () => {
  ...
  const xCordAnimation = useEventWorklet(panWorkletBody, [
    prevX,
    totalX,
    velocityX,
    X,
  ...
  const yCordAnimation = useEventWorklet(panWorkletBody, [
    prevY,
    totalY,
    velocityY,
    Y,
  ...
  return (
    <View
      style={{ flex: 1 }}
      onLayout={e => {
        dimX.set(e.nativeEvent.layout.width);
        dimY.set(e.nativeEvent.layout.height);
      }}>
      <PanGestureHandler
