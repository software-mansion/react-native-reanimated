...
describe('Test _mathematical operations_ on sharedValue', () => {
  ...
  const MultiplySVOriginalReactAPI = <T extends number | bigint>({
    initialValue,
    factor,
    progress,
  }: MultiplyComponentProps<T>) => {
  ...
  async function testSharedValueMultiplication<T extends number | bigint>({
    ...
    const ComponentToRender = mutableAPI === MutableAPI.ORIGINAL ? MultiplySVOriginalAPI : MultiplySVOriginalReactAPI;
