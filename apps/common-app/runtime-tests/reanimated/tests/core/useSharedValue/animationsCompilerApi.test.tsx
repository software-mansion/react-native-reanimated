...
describe(`Test animation assignments on Shared Value using compiler API`, () => {
  ...
  test('WithTiming', async () => {
    ...
    await expectSharedValue(SHARED_VALUE_REF).toBe(100, ComparisonMode.NUMBER);
  });
  ...
  test('WithDecay', async () => {
    ...
    await waitForNotification(NOTIFICATION_NAME);
    await expectSharedValue(SHARED_VALUE_REF).toBe(0, ComparisonMode.NUMBER);
  });
  ...
  test('WithSequence', async () => {
    ...
    await waitForNotification(NOTIFICATION_NAME);
    await expectSharedValue(SHARED_VALUE_REF).toBe(200, ComparisonMode.NUMBER);
  });
});
