...
describe('withSequence animation of array', () => {
  ...
  test.each([
    {
      startValues: [0, 10, 20],
      middleValues: [0, 100, 210],
      finalValues: [20, 10, 30],
      animationNumber: 0,
    },
    {
      startValues: [0, 10, 20],
      middleValues: [0, 150, 160],
      finalValues: [40, 10, 30],
      animationNumber: 1,
    },
    {
      startValues: [0, 10, 20],
    ...
    {
      startValues: [30, 10, 55],
      middleValues: [0, -10, 60],
      finalValues: [40, 10, 30],
      animationNumber: 0,
    },
    {
    ...
    async ({ startValues, middleValues, finalValues, animationNumber }) => {
      ...
      expect(await componentOne.getAnimatedStyle('left')).toBe(
        finalValues[0] + margin,
        ComparisonMode.PIXEL
      );
      expect(await componentTwo.getAnimatedStyle('left')).toBe(
        finalValues[1] + margin,
        ComparisonMode.PIXEL
      );
      expect(await componentThree.getAnimatedStyle('left')).toBe(
        finalValues[2] + margin,
        ComparisonMode.PIXEL
      );
      await waitForNotification(MIDDLE_NOTIFICATION_NAME);
      expect(await componentOne.getAnimatedStyle('left')).toBe(
        middleValues[0] + margin,
        ComparisonMode.PIXEL
      );
      expect(await componentTwo.getAnimatedStyle('left')).toBe(
        middleValues[1] + margin,
        ComparisonMode.PIXEL
      );
      expect(await componentThree.getAnimatedStyle('left')).toBe(
        middleValues[2] + margin,
        ComparisonMode.PIXEL
      );
      await waitForNotification(FINAL_NOTIFICATION_NAME);
      expect(await componentOne.getAnimatedStyle('left')).toBe(
        finalValues[0] + 20 + margin,
        ComparisonMode.PIXEL
      );
      expect(await componentTwo.getAnimatedStyle('left')).toBe(
        finalValues[1] + 20 + margin,
        ComparisonMode.PIXEL
      );
      expect(await componentThree.getAnimatedStyle('left')).toBe(
        finalValues[2] + 20 + margin,
        ComparisonMode.PIXEL
      );
    }
  );
