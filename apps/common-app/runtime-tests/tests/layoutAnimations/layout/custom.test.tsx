...
const moveCornersFirst = (values: LayoutAnimationsValues) => {
  ...
  return {
    animations: {
      width: withSequence(
        withTiming(
          values.currentWidth - values.targetOriginX + values.currentOriginX,
          { duration: 200 }
        ),
      ...
      height: withDelay(
        ...
        withSequence(
          withTiming(
            values.currentHeight - values.targetOriginY + values.currentOriginY,
            { duration: 200 }
          ),
      ...
      originY: withDelay(
        100,
        withTiming(values.targetOriginY, { duration: 200 })
      ),
    },
...
const moveWithRotationOverCorner = (values: LayoutAnimationsValues) => {
  ...
  return {
    animations: {
      ...
      height: withDelay(
        300,
        withTiming(values.targetHeight, { duration: 400 })
      ),
...
const TRANSITIONS = [
  ...
  {
    snapshotPrefix: 'moveWithRotationOverCorner',
    transition: moveWithRotationOverCorner,
  },
  {
    snapshotPrefix: 'colorDependingOnInitialPosition',
    transition: colorDependingOnInitialPosition,
  },
];
...
describe('Test custom layout transitions', () => {
  test.each(TRANSITIONS)(
    'Test animation "${snapshotPrefix}"',
    async ({ snapshotPrefix, transition }) => {
      for (const direction of Object.values(Direction)) {
        const snapshotName = `${snapshotPrefix}_${direction}`;
        const snapshot =
          SnapshotsStaticSize[snapshotName as keyof typeof SnapshotsStaticSize];
  ...
  test.each(TRANSITIONS)(
    ...
    async ({ snapshotPrefix, transition }) => {
      for (const direction of Object.values(Direction)) {
        const snapshotName = `${snapshotPrefix}_${direction}`;
        const snapshot =
          SnapshotsDynamicSize[
            snapshotName as keyof typeof SnapshotsDynamicSize
          ];
        const updates = await getSnapshotUpdates(
          transition,
          direction,
