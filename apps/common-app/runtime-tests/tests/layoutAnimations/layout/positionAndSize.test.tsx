...
describe('Test predefined layout transitions', () => {
  test.each([
    ...
    for (const direction of Object.values(Direction)) {
      const snapshotName =
        `${transition.name}_${direction}` as keyof typeof TransitionSnapshotNoModifiers;
...
describe('Test predefined layout transitions, duration = 200ms', () => {
  test.each([
    ...
    JumpingTransition,
    CurvedTransition,
  ])('Test transition %p, duration = 200ms', async (transition) => {
    for (const direction of Object.values(Direction)) {
      const snapshotName =
        `${transition.name}_${direction}` as keyof typeof TransitionSnapshot200ms;
...
describe('Test **LINEAR** transition modifiers', () => {
  test.each([
    ...
    [
      LinearTransition.springify().duration(450).dampingRatio(0.3),
      'springify3',
    ],
    ...
    async ([transition, modifierName]) => {
      for (const direction of Object.values(Direction)) {
        const snapshotName =
          `LINEAR_${modifierName}_${direction}` as keyof typeof LinearSnapshot;
...
describe('Test **SEQUENCED** transition modifiers', () => {
  test.each([
    ...
    async ([transition, modifierName]) => {
      for (const direction of Object.values(Direction)) {
        const snapshotName =
          `SEQUENCED_${modifierName}_${direction}` as keyof typeof SequencedSnapshot;
...
describe('Test **FADING** transition modifiers', () => {
  test.each([
    ...
    async ([transition, modifierName]) => {
      for (const direction of Object.values(Direction)) {
        const snapshotName =
          `FADING_${modifierName}_${direction}` as keyof typeof FadingSnapshot;
...
describe('Test **JUMPING** transition modifiers', () => {
  test.each([
    ...
    async ([transition, modifierName]) => {
      for (const direction of Object.values(Direction)) {
        const snapshotName =
          `JUMPING_${modifierName}_${direction}` as keyof typeof JumpingSnapshot;
...
describe('Test **CURVED** transition modifiers', () => {
  test.each([
    ...
    [
      CurvedTransition.duration(200)
        .easingX(Easing.bounce)
        .easingY(Easing.bounce),
      'easingBounce_XY',
    ],
    [
      CurvedTransition.duration(200)
        .easingX(Easing.steps(7))
        .easingY(Easing.steps(7)),
      'easingSteps_7_7',
    ],
    [
      CurvedTransition.duration(200)
        .easingX(Easing.steps(7))
        .easingY(Easing.steps(5)),
      'easingSteps_7_5',
    ],
  ] as Array<[any, string]>)(
    'Modifiers: ${1}',
    async ([transition, modifierName]) => {
      for (const direction of Object.values(Direction)) {
        const snapshotName =
          `CURVED_${modifierName}_${direction}` as keyof typeof CurvedSnapshot;
