...
async function getSnapshotUpdates(
  ...
  const componentEntering = duration
    ? springEntering.duration(duration)
    : springEntering;
...
describe('Test predefined entering', () => {
  describe('Entering on mount, no modifiers', () => {
    test.each(ENTERING_SETS)(
      'Test suite of ${0}In',
      async ([_setName, enteringSet]) => {
        for (const entering of enteringSet) {
          const snapshotName = (entering as any)
            .name as keyof typeof NoModifierEnteringSnapshots;
  ...
  describe('Entering on mount, duration 100', () => {
    test.each(ENTERING_SETS)(
      ...
      async ([_setName, enteringSet]) => {
        for (const entering of enteringSet) {
          const enteringName = (entering as any)
            .name as keyof typeof DurationEnteringSnapshots;
  ...
  describe('Entering on mount, springify', () => {
    test.each(ENTERING_SETS)(
      ...
      async ([_setName, enteringSet]) => {
        for (const entering of enteringSet) {
          const snapshotName = (entering as any)
            .name as keyof typeof SpringifyEnteringSnapshots;
