#!/usr/bin/env node
/**
 * Host-side runner for the on-device runtime tests.
 *
 * Usage:
 ...
 *   --platform        Target platform. `ios` (default) drives a simulator via
 *                     xcodebuild/simctl; `android` drives a device/emulator via
 *                     gradle/adb. Android supports only Debug configurations.
 *   --launch          Also build, install and launch the app on the target
 *                     after the server is listening. Without it, the server
 *                     just waits for a device to connect.
 *   --skip-build      With --launch: skip the native build and reuse the app
 *                     that is already installed (or in the build outputs).
 *   --only            Comma-separated suite filter forwarded in the `start`
 *                     envelope.
 *   --metro-port      Metro port to probe/start. Defaults to 8081. On Android
 *                     a non-default port is baked into the APK at build time,
 *                     so it only takes effect when the app is (re)built.
 ...
 *   --serial          Android device serial from `adb devices`. Defaults to
 *                     the first connected device; when none is connected the
 *                     first AVD is booted.
 *   --avd             AVD name to boot when no Android device is connected.
 *   --connect-timeout Seconds to wait for the first client, counted from app
 *                     launch when --launch is set (so a slow native build does
...
async function buildAndroidApp(serial) {
  // Building only the target device's ABI cuts the native build time roughly
  // in four compared to the default four-ABI matrix.
...
async function installAndLaunchAndroid(serial) {
  ...
  // Emulators reach the host directly through 10.0.2.2; the reverses matter
  // only for physical devices, which resolve the dev server as localhost.
