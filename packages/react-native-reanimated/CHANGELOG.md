...
- Fix fractional millisecond CSS time values being truncated to a whole millisecond, or parsed as `NaN` when written without a leading zero, so that `0.5ms` no longer becomes `0` and `.5ms` no longer reaches native. ([#10381](https://github.com/software-mansion/react-native-reanimated/pull/10381) by [@dennytosp](https://github.com/dennytosp))
