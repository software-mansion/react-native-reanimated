...
- Fix importing Reanimated under Jest crashing at initialization with ``[Reanimated] `setCSSEventHandler` is not available in JSReanimated.`` when the Jest resolver is not configured - the native-only initialization is skipped and a warning points to the resolver setup. ([#10377](https://github.com/software-mansion/react-native-reanimated/pull/10377) by [@huextrat](https://github.com/huextrat))
