...
- Ship the Jest resolver (`react-native-reanimated/jest/resolver`) in the npm package, so consumer projects can run Jest against Reanimated: the modules that require a real native module are resolved to their web implementations. (by [@huextrat](https://github.com/huextrat))
- Fix importing Reanimated under Jest crashing at initialization with ``[Reanimated] `setCSSEventHandler` is not available in JSReanimated.`` when the Jest resolver is not configured - the native-only initialization is skipped and a warning points to the resolver setup. (by [@huextrat](https://github.com/huextrat))
- Fix mounting an animated component with attached event handlers (e.g. `useAnimatedScrollHandler`) under Jest crashing with ``[Reanimated] registerEventHandler is not available in JSReanimated.`` - `WorkletEventHandler` now resolves to its web variant in Jest. (by [@huextrat](https://github.com/huextrat))
