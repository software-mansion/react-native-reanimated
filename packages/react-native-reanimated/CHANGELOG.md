...
- Feed synchronous prop updates into the experimental layout animations proxy's light tree behind the `SYNCHRONOUS_PROPS_IN_LIGHT_TREE` dynamic feature flag, so shared element transitions and layout animations start from the current props of a view that moved through the synchronous path; warn in development when a transition or layout animation starts on such a view with the flag off. The synchronous-update feature flags now work together with `ENABLE_SHARED_ELEMENT_TRANSITIONS`. (PR_B_LINK by [@pawicao](https://github.com/pawicao))
