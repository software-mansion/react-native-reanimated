...
public class NativeProxy {
  ...
  private final Map<Integer, Runnable> mPseudoSelectorDetachActions = new HashMap<>();
  ...
  @DoNotStrip
  public void attachPseudoSelector(int tag, String selector, PseudoSelectorCallback callback) {
    UiThreadUtil.runOnUiThread(
        () -> {
          View view = mFabricUIManager.resolveView(tag);
          if (view == null) {
            return;
          }
          if (":focus".equals(selector)) {
            ...
            mPseudoSelectorDetachActions.put(tag, () -> view.setOnTouchListener(null));
          }
        });
  }

  @DoNotStrip
  public void detachPseudoSelector(int tag) {
    UiThreadUtil.runOnUiThread(
        () -> {
          Runnable detach = mPseudoSelectorDetachActions.remove(tag);
          if (detach != null) {
            detach.run();
          }
        });
