...
static CAMediaTimingFunction *getCATimingFunction(id easing)
{
  if ([easing isKindOfClass:[NSArray class]]) {
    ...
    return [CAMediaTimingFunction functionWithControlPoints:
        [points[0] floatValue] :[points[1] floatValue]
        :[points[2] floatValue] :[points[3] floatValue]];
  }
...
static id getCAValue(NSString *keyPath, id rawValue)
{
  if ([keyPath isEqualToString:@"transform"]) {
    ...
    CATransform3D t;
    t.m11 = [arr[0] doubleValue]; t.m12 = [arr[1] doubleValue]; t.m13 = [arr[2] doubleValue]; t.m14 = [arr[3] doubleValue];
    t.m21 = [arr[4] doubleValue]; t.m22 = [arr[5] doubleValue]; t.m23 = [arr[6] doubleValue]; t.m24 = [arr[7] doubleValue];
    t.m31 = [arr[8] doubleValue]; t.m32 = [arr[9] doubleValue]; t.m33 = [arr[10] doubleValue]; t.m34 = [arr[11] doubleValue];
    t.m41 = [arr[12] doubleValue]; t.m42 = [arr[13] doubleValue]; t.m43 = [arr[14] doubleValue]; t.m44 = [arr[15] doubleValue];
...
static CAKeyframeData extractKeyframeValues(
    NSString *keyPath,
    NSArray *keyframes,
    BOOL isReversed)
{
    ...
    BOOL isReversed)
{
  ...
  if (easings.count > 0) {
    ...
    for (NSDictionary *easing in easingsToUse) {
...
static CAAnimation *buildCAAnimation(NSDictionary *descriptor, NSDictionary *settings)
{
  ...
  CAAnimation *animation = createAnimationFromKeyframes(
      keyPath, keyframeData, descriptor[@"easings"], isReversed);
...
- (void)applyCSSAnimationsForView:(REAUIView *)view
                       animations:(NSArray *)animations
{
  ...
  [self removeStaleAnimationsFromLayer:layer
                               viewKey:viewKey
...
{
  [self removeStaleAnimationsFromLayer:view.layer
                               viewKey:@(view.tag)
                          orderedNames:[NSSet set]];
}
