...
std::optional<MountingTransaction> LayoutAnimationsProxy::pullTransaction(
  ...
  //            const auto tag = mutations[i].newChildShadowView.tag;
  //            const auto &oldFrame =
  //            mutations[i].oldChildShadowView.layoutMetrics.frame; const auto
  //            &newFrame = mutations[i].newChildShadowView.layoutMetrics.frame;
  //
  //            if (oldFrame.origin.y != newFrame.origin.y) {
  ////                LOG(INFO) << "Item " << i + 1 << ": Old(" <<
  ///oldFrame->origin.x << ", " << oldFrame->origin.y << "), New(" <<
  ///newFrame->origin.x << ", " << newFrame->origin.y << ")";
  //                runCoreAnimationForView_(tag, oldFrame, newFrame, [this,
  //                tag]{
...
void LayoutAnimationsProxy::startLayoutAnimation(
  ...
  // TODO: Here or inside the scheduleOnUI?
  //  runCoreAnimationForView_(
  //      tag, mutation.oldChildShadowView.layoutMetrics.frame,
  //      mutation.newChildShadowView.layoutMetrics.frame, [this, tag]{
  //                              this->endLayoutAnimation(tag, false);
  //  });
  ...
  uiScheduler_->scheduleOnUI(
      [weakThis = weak_from_this(), mutation, surfaceId, tag, this]() {
        ...
        runCoreAnimationForView_(
            ...
            [this, tag] { this->endLayoutAnimation(tag, false); });
      });
}
