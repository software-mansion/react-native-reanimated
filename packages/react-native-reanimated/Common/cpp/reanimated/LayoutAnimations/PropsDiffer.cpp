...
void PropsDiffer::diffTransform(jsi::Runtime& rt) {
  ...
  if (
    sourceJsiOperations.size() == 1 && targetJsiOperations.size() == 1
...
std::vector<TransformOperationWithDefault> PropsDiffer::getTransformOperationsFromProps(jsi::Runtime& rt, const ViewProps& props) {
...
void PropsDiffer::maybeAddOperationToDiff(jsi::Runtime& rt, const char* name, float value, float defaultValue, std::vector<TransformOperationWithDefault>& jsiOperations) {
...
void PropsDiffer::diffTransformOrigin(jsi::Runtime& rt) {
  ...
  if (
      sourceTransformOrigin.xy[0] == targetTransformOrigin.xy[0] &&
...
void PropsDiffer::addTransformOriginToDiff(jsi::Runtime& rt, const TransformOrigin& transformOrigin, jsi::Object& jsiValues, const ShadowView& view) {
...
void PropsDiffer::diffBorder(jsi::Runtime& rt) {
  ...
  diffBorderWidth(sourceBorderWidths.all, targetBorderWidths.all, "borderWidth", rt, defaultSourceWidth, defaultTargetWidth);
  diffBorderWidth(sourceBorderWidths.left, targetBorderWidths.left, "borderLeftWidth", rt, defaultSourceWidth, defaultTargetWidth);
  diffBorderWidth(sourceBorderWidths.right, targetBorderWidths.right, "borderRightWidth", rt, defaultSourceWidth, defaultTargetWidth);
  diffBorderWidth(sourceBorderWidths.top, targetBorderWidths.top, "borderTopWidth", rt, defaultSourceWidth, defaultTargetWidth);
  diffBorderWidth(sourceBorderWidths.bottom, targetBorderWidths.bottom, "borderBottomWidth", rt, defaultSourceWidth, defaultTargetWidth);
...
void PropsDiffer::diffBorderRadius(const std::optional<react::ValueUnit>& sourceValue, const std::optional<react::ValueUnit>& targetValue, const char* name, jsi::Runtime& rt) {
  ...
  // In React Native, we can't set asymmetric border radii for the edges of a corner using pixels.
  // This limitation means we can't properly animate between units in percentages and pixels.
...
void PropsDiffer::diffBorderWidth(const std::optional<react::Float>& sourceValue, const std::optional<react::Float>& targetValue, const char* name, jsi::Runtime& rt, float defaultSourceWidth, float defaultTargetWidth) {
...
void PropsDiffer::diffBorderColors(const std::optional<react::SharedColor>& sourceValue, const std::optional<react::SharedColor>& targetValue, const char* name, jsi::Runtime& rt) {
