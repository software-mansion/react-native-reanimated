...
void PropsDiffer::overrideTransforms(const Transform &sourceTransform, const Transform &targetTransform) {
  sourceTransform_ = sourceTransform;
  targetTransform_ = targetTransform;
  needsOverrideTransforms_ = true;
}
...
void PropsDiffer::diffBackgroundColor(jsi::Runtime &rt) {
  if (sourceViewProps_.backgroundColor != targetViewProps_.backgroundColor) {
    sourceValues_.setProperty(
        rt,
        "backgroundColor",
        react::toString(sourceViewProps_.backgroundColor));
    targetValues_.setProperty(
        rt,
        "backgroundColor",
        react::toString(targetViewProps_.backgroundColor));
  }
}

void PropsDiffer::diffTransform(jsi::Runtime &rt) {

  if (needsOverrideTransforms_) {
  ...
  const auto &targetJsiOperations =
      getTransformOperationsFromProps(rt, targetViewProps_);
  ...
  if (sourceJsiOperations.size() == 1 && targetJsiOperations.size() == 1 &&
    ...
    sourceTransforms.setValueAtIndex(
        rt, 0, sourceJsiOperations[0].currentValue);
    targetTransforms.setValueAtIndex(
        rt, 0, targetJsiOperations[0].currentValue);
    sourceValues_.setProperty(rt, "transform", sourceTransforms);
    targetValues_.setProperty(rt, "transform", targetTransforms);
    return;
  }

  jsi::Array sourceTransforms(
      rt, sourceJsiOperations.size() + targetJsiOperations.size());
  jsi::Array targetTransforms(
      rt, sourceJsiOperations.size() + targetJsiOperations.size());
  for (size_t i = 0; i < sourceJsiOperations.size(); i++) {
    sourceTransforms.setValueAtIndex(
        rt, i, sourceJsiOperations[i].currentValue);
    targetTransforms.setValueAtIndex(
        rt, i, sourceJsiOperations[i].defaultValue);
  }
  for (size_t i = 0; i < targetJsiOperations.size(); i++) {
    size_t index = i + sourceJsiOperations.size();
    sourceTransforms.setValueAtIndex(
        rt, index, targetJsiOperations[i].defaultValue);
    targetTransforms.setValueAtIndex(
        rt, index, targetJsiOperations[i].currentValue);
  }
  sourceValues_.setProperty(rt, "transform", sourceTransforms);
  targetValues_.setProperty(rt, "transform", targetTransforms);
}
...
PropsDiffer::getTransformOperationsFromProps(
    jsi::Runtime &rt,
  ...
  if (operations.size() == 1 &&
      operations[0].type == react::TransformOperationType::Arbitrary) {
  ...
  for (const auto &operation : operations) {
    switch (operation.type) {
      case react::TransformOperationType::Perspective: {
        maybeAddOperationToDiff(
            rt, "perspective", operation.x.value, 1, jsiOperations);
      } break;

      case react::TransformOperationType::Scale: {
        maybeAddOperationToDiff(
            rt, "scaleX", operation.x.value, 1, jsiOperations);
        maybeAddOperationToDiff(
            rt, "scaleY", operation.y.value, 1, jsiOperations);
        maybeAddOperationToDiff(
            rt, "scaleZ", operation.z.value, 1, jsiOperations);
      } break;

      case react::TransformOperationType::Translate: {
        maybeAddOperationToDiff(
            rt, "translateX", operation.x.value, 0, jsiOperations);
        maybeAddOperationToDiff(
            rt, "translateY", operation.y.value, 0, jsiOperations);
        maybeAddOperationToDiff(
            rt, "translateZ", operation.z.value, 0, jsiOperations);
      } break;

      case react::TransformOperationType::Rotate: {
        maybeAddOperationToDiff(
            rt, "rotateX", operation.x.value, 0, jsiOperations);
        maybeAddOperationToDiff(
            rt, "rotateY", operation.y.value, 0, jsiOperations);
        maybeAddOperationToDiff(
            rt, "rotateZ", operation.z.value, 0, jsiOperations);
      } break;

      case react::TransformOperationType::Skew: {
        maybeAddOperationToDiff(
            rt, "skewX", operation.x.value, 0, jsiOperations);
        maybeAddOperationToDiff(
            rt, "skewY", operation.y.value, 0, jsiOperations);
        maybeAddOperationToDiff(
            rt, "skewZ", operation.z.value, 0, jsiOperations);
      } break;
...
void PropsDiffer::diffTransformOrigin(jsi::Runtime &rt) {
  ...
  addTransformOriginToDiff(
      rt, sourceTransformOrigin, sourceValues_, sourceView_);
  addTransformOriginToDiff(
      rt, targetTransformOrigin, targetValues_, targetView_);
}
...
void PropsDiffer::addTransformOriginToDiff(
  ...
  if (transformOrigin.xy[0].unit == UnitType::Percent) {
    const float origin =
        view.layoutMetrics.frame.size.width * transformOrigin.xy[0].value / 100;
  ...
  if (transformOrigin.xy[1].unit == UnitType::Percent) {
    const float origin = view.layoutMetrics.frame.size.height *
        transformOrigin.xy[1].value / 100;
...
void PropsDiffer::diffShadow(jsi::Runtime &rt) {
  ...
  if (sourceBoxShadows.size() == targetBoxShadows.size()) {
    ...
    for (int i = 0; i < size; i++) {
      sourceBoxShadowArray.setValueAtIndex(
          rt, i, sourceBoxShadows[i].currentValue);
      targetBoxShadowArray.setValueAtIndex(
          rt, i, targetBoxShadows[i].currentValue);
    }
    ...
    for (size_t i = 0; i < sourceBoxShadows.size(); i++) {
      sourceBoxShadowArray.setValueAtIndex(
          rt, i, sourceBoxShadows[i].currentValue);
      targetBoxShadowArray.setValueAtIndex(
          rt, i, sourceBoxShadows[i].defaultValue);
    }
    for (size_t i = 0; i < targetBoxShadows.size(); i++) {
      size_t index = i + sourceBoxShadows.size();
      sourceBoxShadowArray.setValueAtIndex(
          rt, index, targetBoxShadows[i].defaultValue);
      targetBoxShadowArray.setValueAtIndex(
          rt, index, targetBoxShadows[i].currentValue);
    }
    sourceValues_.setProperty(rt, "boxShadow", sourceBoxShadowArray);
    targetValues_.setProperty(rt, "boxShadow", targetBoxShadowArray);
  }
  ...
  if (sourceViewProps_.shadowColor != targetViewProps_.shadowColor) {
    sourceValues_.setProperty(
        rt, "shadowColor", toString(sourceViewProps_.shadowColor));
    targetValues_.setProperty(
        rt, "shadowColor", toString(targetViewProps_.shadowColor));
  }
  if (sourceViewProps_.shadowOffset != targetViewProps_.shadowOffset) {
    jsi::Object sourceShadowOffset(rt), targetShadowOffset(rt);
    sourceShadowOffset.setProperty(
        rt, "width", sourceViewProps_.shadowOffset.width);
    sourceShadowOffset.setProperty(
        rt, "height", sourceViewProps_.shadowOffset.height);
    targetShadowOffset.setProperty(
        rt, "width", targetViewProps_.shadowOffset.width);
    targetShadowOffset.setProperty(
        rt, "height", targetViewProps_.shadowOffset.height);
  ...
  if (sourceViewProps_.shadowOpacity != targetViewProps_.shadowOpacity) {
    sourceValues_.setProperty(
        rt, "shadowOpacity", sourceViewProps_.shadowOpacity);
    targetValues_.setProperty(
        rt, "shadowOpacity", targetViewProps_.shadowOpacity);
  }
  if (sourceViewProps_.shadowRadius != targetViewProps_.shadowRadius) {
    sourceValues_.setProperty(
        rt, "shadowRadius", sourceViewProps_.shadowRadius);
    targetValues_.setProperty(
        rt, "shadowRadius", targetViewProps_.shadowRadius);
  }
#ifdef ANDROID
...
std::vector<BoxShadowWithDefault> PropsDiffer::getBoxShadowsFromProps(
    jsi::Runtime &rt,
    const ViewProps &props) {
  ...
  for (const auto &boxShadow : boxShadows) {
    ...
    currentBoxShadow.setProperty(
        rt, "spreadDistance", boxShadow.spreadDistance);
...
void PropsDiffer::diffBorder(jsi::Runtime &rt) {
  ...
  diffBorderRadius(
      sourceBorderRadii.all, targetBorderRadii.all, "borderRadius", rt);
  diffBorderRadius(
  ...
  diffBorderColors(
      sourceBorderColors.all, targetBorderColors.all, "borderColor", rt);
  diffBorderColors(
      sourceBorderColors.left, targetBorderColors.left, "borderLeftColor", rt);
  diffBorderColors(
      ...
      rt);
  diffBorderColors(
      sourceBorderColors.top, targetBorderColors.top, "borderTopColor", rt);
  diffBorderColors(
...
void PropsDiffer::diffBorderRadius(
  ...
  if (targetValue.value_or(defaultValue).unit == UnitType::Percent) {
    if (sourceValue.has_value()) {
      ...
      const auto &defaultRadius =
          sourceViewProps_.borderRadii.all.value_or(defaultValue).value;
    ...
    if (targetValue.has_value()) {
      ...
      const auto &defaultRadius =
          targetViewProps_.borderRadii.all.value_or(defaultValue).value;
...
void PropsDiffer::diffBorderColors(
  ...
  if (source != target) {
    if (sourceValue.has_value()) {
      ...
      auto const &maybeDefaultColor =
          sourceViewProps_.borderColors.all.value_or(defaultValue);
    ...
    if (targetValue.has_value()) {
      ...
      auto const &maybeDefaultColor =
          targetViewProps_.borderColors.all.value_or(defaultValue);
