...
std::shared_ptr<LayoutAnimationsProxyCommon> LayoutAnimationsProxyRegistry::registerSurface(
  ...
  const auto lock = std::lock_guard<std::mutex>(instancesMutex_);
...
std::optional<SurfaceId> LayoutAnimationsProxyRegistry::onGestureCancel() {
  for (const auto &instance : instances()) {
    if (const auto surfaceId = instance->onGestureCancel()) {
      return surfaceId;
    }
  }
  return {};
...
std::vector<std::shared_ptr<LayoutAnimationsProxyCommon>> LayoutAnimationsProxyRegistry::instances() const {
  ...
  for (const auto &[surfaceId, instance] : instances_) {
