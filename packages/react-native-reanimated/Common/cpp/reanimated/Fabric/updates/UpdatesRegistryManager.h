...
class UpdatesRegistryManager {
 ...
 private:
  using RemovableShadowNodes = std::unordered_map<Tag, std::shared_ptr<ShadowNodeFamily>>;
  ...
  static void
  addToPropsMap(PropsMap &propsMap, const std::shared_ptr<ShadowNodeFamily> &shadowNodeFamily, const folly::dynamic &props);
#endif
};
