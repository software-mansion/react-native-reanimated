...
double OperationsLoop::resolveTimestamp() {
  ...
  // Slow path: race for the fill. compare_exchange_strong has a useful side
  // effect on failure - it atomically loads the current cache value into
  // `expected`, so concurrent losers naturally pick up the winner's timestamp
  // and all callers in the same fill cycle return the same value.
