...
std::optional<std::pair<StyleOperations, StyleOperations>> TransformsStyleInterpolator::createInterpolationPair(
  ...
  while (!shouldInterpolateMatrices && i < fromOperations.size() && j < toOperations.size()) {
    ...
    if (fromType == toType) {
      ...
      const bool toExistsLaterInFrom = lastIndexInFrom.count(toOperation->type) && lastIndexInFrom[toOperation->type] > i;
      const bool fromExistsLaterInTo = lastIndexInTo.count(fromOperation->type) && lastIndexInTo[fromOperation->type] > j;
