...
folly::dynamic TransformsStyleInterpolator::getResetStyle(const std::shared_ptr<const ShadowNode> &shadowNode) const {
...
void TransformsStyleInterpolator::updateKeyframes(jsi::Runtime &rt, const jsi::Value &keyframes) {
...
std::optional<TransformOperations> TransformsStyleInterpolator::parseTransformOperations(
    jsi::Runtime &rt,
  ...
  for (size_t i = 0; i < transformsCount; ++i) {
    ...
    transformOperations.emplace_back(TransformOperation::fromJSIValue(rt, transform));
  }
  return transformOperations;
}

std::optional<TransformOperations> TransformsStyleInterpolator::parseTransformOperations(const folly::dynamic &values) {
  if (values.empty()) {
    return std::nullopt;
  }
...
std::shared_ptr<TransformKeyframe> TransformsStyleInterpolator::createTransformKeyframe(
  ...
  if (!fromOperationsOptional.has_value() || !toOperationsOptional.has_value()) {
    return std::make_shared<TransformKeyframe>(
        TransformKeyframe{fromOffset, toOffset, fromOperationsOptional, toOperationsOptional});
  }
  ...
  return std::make_shared<TransformKeyframe>(TransformKeyframe{fromOffset, toOffset, fromOperations, toOperations});
}
...
std::shared_ptr<TransformOperation> TransformsStyleInterpolator::getDefaultOperationOfType(
...
folly::dynamic TransformsStyleInterpolator::interpolateOperations(
  ...
  const auto transformUpdateContext = TransformInterpolationContext{shadowNode, viewStylesRepository_, interpolators_};
  ...
  for (size_t i = 0; i < fromOperations.size(); ++i) {
    ...
    result.emplace_back(
        interpolator->interpolate(keyframeProgress, fromOperation, toOperation, transformUpdateContext));
  }
...
folly::dynamic TransformsStyleInterpolator::convertOperationsToDynamic(const TransformOperations &operations) {
