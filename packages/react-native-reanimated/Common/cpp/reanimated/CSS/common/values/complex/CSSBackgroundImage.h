...
// A single gradient color stop. The color is monostate for transition hints
// (e.g. the "20%" in "red, 20%, blue"). A color React Native resolves natively
// (PlatformColor, DynamicColorIOS) is kept as the raw payload: such a stop
// cannot be interpolated, so the gradient switches discretely, but it still
// reaches the renderer intact. The position is std::nullopt when it's not
// specified (React Native distributes such stops evenly).
struct GradientColorStop {
  using Color = std::variant<std::monostate, CSSColor, folly::dynamic>;

  Color color;
  std::optional<GradientLengthPercentage> position;

  bool isHint() const;
  bool hasNativeColor() const;
