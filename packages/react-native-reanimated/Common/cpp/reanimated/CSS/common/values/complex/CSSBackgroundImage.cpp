...
GradientColorStop GradientColorStop::fromDynamic(const folly::dynamic &dynValue) {
  ...
  if (colorIt != dynValue.items().end() && !colorIt->second.isNull()) {
    if (CSSColor::canConstruct(colorIt->second)) {
      result.color = CSSColor(colorIt->second);
    } else {
      result.color = colorIt->second;
    }
  }
...
bool GradientColorStop::isHint() const {
  return std::holds_alternative<std::monostate>(color);
}

bool GradientColorStop::hasNativeColor() const {
  return std::holds_alternative<folly::dynamic>(color);
}

folly::dynamic GradientColorStop::toDynamic() const {
  folly::dynamic obj = folly::dynamic::object();
  if (const auto *cssColor = std::get_if<CSSColor>(&color)) {
    obj["color"] = cssColor->toDynamic();
  } else if (const auto *nativeColor = std::get_if<folly::dynamic>(&color)) {
    obj["color"] = *nativeColor;
  } else {
    obj["color"] = folly::dynamic();
  }
...
std::string GradientColorStop::toString() const {
  std::string result;
  if (const auto *cssColor = std::get_if<CSSColor>(&color)) {
    result = cssColor->toString();
  } else if (const auto *nativeColor = std::get_if<folly::dynamic>(&color)) {
    result = folly::toJson(*nativeColor);
  }
...
GradientColorStop GradientColorStop::interpolate(double progress, const GradientColorStop &to) const {
  GradientColorStop result;
  const auto *fromColor = std::get_if<CSSColor>(&color);
  const auto *toColor = std::get_if<CSSColor>(&to.color);
  if (fromColor != nullptr && toColor != nullptr) {
    result.color = fromColor->interpolate(progress, *toColor);
  }
...
bool GradientColorStop::canInterpolateTo(const GradientColorStop &to) const {
  if (hasNativeColor() || to.hasNativeColor()) {
    return false;
  }
  if (isHint() != to.isHint()) {
    return false;
...
bool areColorStopsCompatible(const std::vector<GradientColorStop> &from, const std::vector<GradientColorStop> &to) {
  // A gradient without stops is the placeholder for an absent layer.
