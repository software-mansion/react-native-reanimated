...
static jsi::Value REANIMATED_SPEC_PREFIX(registerNativePropsForView)(
    jsi::Runtime &rt,
    TurboModule &turboModule,
    const jsi::Value *args,
    size_t) {
  static_cast<ReanimatedModuleProxySpec *>(&turboModule)
      ->registerNativePropsForView(rt, std::move(args[0]), std::move(args[1]));
  return jsi::Value::undefined();
}
...
ReanimatedModuleProxySpec::ReanimatedModuleProxySpec(
  ...
  methodMap_["registerNativePropsForView"] =
      MethodMetadata{2, REANIMATED_SPEC_PREFIX(registerNativePropsForView)};
