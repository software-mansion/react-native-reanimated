...
export default class AnimatedComponent extends ReanimatedAnimatedComponent {
  ...
  constructor(ChildComponent, props, displayName, options) {
    ...
    if (!entering || getReducedMotionFromConfig(entering) || skipEntering) {
      return;
    }
    // This call is responsible for configuring entering animations on Fabric.
    updateLayoutAnimations(this.reanimatedID, LayoutAnimationType.ENTERING, maybeBuild(entering, this.props?.style, displayName));
  }
  componentDidMount() {
  ...
  _attachAnimatedStyles() {
    ...
    const hasReanimated2Props = this.props.animatedProps?.viewDescriptors || this._animatedStyles?.length;
  ...
  _configureLayoutTransition() {
    ...
    updateLayoutAnimations(this.getComponentViewTag(), LayoutAnimationType.LAYOUT, layout && maybeBuild(layout, undefined /* We don't have to warn user if style has common properties with animation for LAYOUT */, this._displayName));
  }
  _onSetLocalRef() {
    ...
    if (layout || entering || exiting) {
      ...
      if (exiting) {
        ...
        if (!reduceMotionInExiting) {
          updateLayoutAnimations(tag, LayoutAnimationType.EXITING, maybeBuild(exiting, this.props?.style, this._displayName));
        }
      }
    }
  }
