...
export default class CSSAnimationsManager {
  static animationKeyframesRegistry = new CSSKeyframesRegistry();
  attachedAnimations = [];
  ...
  registerKeyframesUsage(processedAnimations) {
    ...
    processedAnimations.forEach(({
      ...
      CSSAnimationsManager.animationKeyframesRegistry.add(keyframesRule, this.viewTag);
    ...
    this.attachedAnimations.forEach(({
      ...
      if (!newAnimationNames.has(name)) {
        CSSAnimationsManager.animationKeyframesRegistry.remove(name, this.viewTag);
      }
    });
  }
  ...
  processAnimations(animationProperties) {
    ...
    const processedAnimations = singleAnimationPropertiesArray.map((properties, i) => {
      ...
      if (keyframes instanceof CSSKeyframesRuleImpl) {
      ...
      } else if (this.attachedAnimations[i]?.keyframesRule.cssText !== JSON.stringify(keyframes)) {
        ...
        keyframesRule = new CSSKeyframesRuleImpl(keyframes);
      } else {
