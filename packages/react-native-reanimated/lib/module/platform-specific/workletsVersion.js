...
import { acceptedWorkletsVersion } from "./jsVersion.js";
export function assertWorkletsVersion() {
  ...
  try {
    ...
    throw new ReanimatedError("react-native-worklets package isn't installed. Please install it to use Reanimated.");
  }
  ...
  if (!semverSatisfies(workletsVersion, acceptedRange)) {
    throw new ReanimatedError(`Invalid version of react-native-worklets: ${workletsVersion}. Expected the version to be in inclusive range ${acceptedRange}. Please install a compatible version of react-native-worklets.`);
  }
}
