  ...
  describe('update', () => {
    ...
    describe('transitions (runCSSTransition payload)', () => {
      ...
      describe('detach via null config', () => {
        ...
        // The props are still present, but the 0ms config normalizes to empty.
        test('unregisters when the config normalizes to empty (0ms)', () => {
          manager.update(DEFAULT_TRANSITION_CONFIG, { opacity: 0 });
          manager.update(DEFAULT_TRANSITION_CONFIG, { opacity: 1 });
          jest.clearAllMocks();
        ...
        test('does not unregister when no transition has run', () => {
          manager.update(DEFAULT_TRANSITION_CONFIG, { opacity: 0 });
          jest.clearAllMocks();
