...
export function enableSvgInheritedProps() {
  enabled = true;
}
...
// only when the element received it in JS, since that is what it builds
// `propList` from. A CSS animation or pseudo state writes to the shadow node
// directly, so a prop it is the only source of has to be passed inline too,
// at the value react-native-svg draws with when it is unset. Owning the prop
// also stops an ancestor from overwriting the animated value on every draw.
export function addSvgInheritedProps(
  ...
  for (const entry of animatedProps) {
    ...
    for (const key in entry) {
      if (key === 'animationName') {
        collectFromKeyframes(entry[key], (inherited ??= new Set()));
      } else if (
        !isCSSConfigProp(key) &&
