...
- Fix the Jest mock's `createShareable` ignoring the host and guest decorators, which left Reanimated mutables built on top of it without `value`, `modify` and the listeners API. (by [@huextrat](https://github.com/huextrat))
