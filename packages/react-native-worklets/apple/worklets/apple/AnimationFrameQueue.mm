...
@implementation AnimationFrameQueue{
  WorkletsDisplayLink *_displayLink;
//  NSMutableArray<AnimationFrameCallback> *_frameCallbacks;
  std::vector<std::function<void(double)>> _frameCallbacks;
}

- (instancetype)init{
  _displayLink = [WorkletsDisplayLink displayLinkWithTarget:self selector:@selector(executeQueue:)];
#if TARGET_OS_OSX
  // nothing
#else // TARGET_OS_OSX
...
{
//  [_frameCallbacks addObject:^(WorkletsDisplayLink *displayLink){
//    
//  }];
...
//  }
  ...
  // TODO: MUTEX HERE
//  NSArray<AnimationFrameCallback> *frameCallbacks = _frameCallbacks;
//  _frameCallbacks = [NSMutableArray new];
  
  // TODO: enhance
  auto targetTimestamp = displayLink.targetTimestamp;
  
  auto frameCallbacks = std::move(_frameCallbacks);
  _frameCallbacks.clear();
  
  [_displayLink setPaused:TRUE];
  
//  for (AnimationFrameCallback callback in frameCallbacks) {
//    callback(displayLink);
//  }
  
//  for (std::function<void(double)> callback in frameCallbacks) {
//  }
