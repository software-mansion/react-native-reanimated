 ...
 * object.
 *
 * The host carries `__closure` — the captured values, in the order the emitted
 * destructure names them — and whatever else a test hands the worklet to
 ...
 */
type WorkletHost = {
  __closure: ReadonlyArray<unknown>;
...
describe('babel plugin', () => {
  ...
  describe('closure-dependent parameters', () => {
    ...
    test('the emitted worklet RUNS, and runs its defaults in source order', () => {
      ...
      const result = worklet.call({ __closure: [{ max: 8 }], trace });
    ...
    test('a body statement calling a `__classFactory` name the plugin did not generate is not a hoist anchor', () => {
      ...
      expect(code).toContain(
        'const[DEFAULTS,gadget__classFactory]=this.__closure;'
      );
