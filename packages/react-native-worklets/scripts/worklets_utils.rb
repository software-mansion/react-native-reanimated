...
def worklets_assert_minimal_react_native_version(config)
  ...
  unless system("node \"#{validate_react_native_version_script}\" #{config[:react_native_version]}")
    raise "[Worklets] React Native version is not compatible with Worklets. See the documentation for the list of supported versions https://docs.swmansion.com/react-native-worklets/docs/guides/compatibility"
  end
end
