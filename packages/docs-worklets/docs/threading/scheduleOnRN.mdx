...
`scheduleOnRN` is commonly used to update React state from [UI Runtime](/docs/fundamentals/glossary#ui-runtime), e.g. when animation finishes or conditionally within a gesture.
...
- Functions passed to `scheduleOnRN` must be defined in the [RN Runtime](/docs/fundamentals/glossary#react-native-runtime-rn-runtime) scope, i.e. in the component body or the global scope. The code below won't work because `myFunction` is defined in the `runOnUI` callback, which is only executed in the [UI Runtime](/docs/fundamentals/glossary#ui-runtime):
...
- When called from a [Worker Runtime](/docs/fundamentals/runtimeKinds#worker-worklet-runtime---worker-runtime), it uses the same JSScheduler mechanism.
