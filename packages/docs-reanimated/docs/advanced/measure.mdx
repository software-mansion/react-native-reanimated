...
`measure` lets you synchronously get the dimensions and position of a view on the screen, all on the [UI thread](/docs/next/fundamentals/glossary#ui-thread).
...
An [animated ref](/docs/next/core/useAnimatedRef#returns) connected to the component you'd want to get the measurements from. The animated ref has to be passed either to an [Animated component](/docs/next/fundamentals/glossary#animated-component) or a React Native built-in component.
...
- `measure` is implemented only on the [UI thread](/docs/next/fundamentals/glossary#ui-thread). When using `measure` inside [event handlers](https://react.dev/learn/responding-to-events#adding-event-handlers), it has to be wrapped with the [`runOnUI`](/docs/next/threading/runOnUI) function.

- The `useAnimatedStyle` function is first evaluated on the [JavaScript thread](/docs/next/fundamentals/glossary#javascript-thread) just before the views are attached to the native side. For this reason, to safely use the measure within `useAnimatedStyle`, a condition similar to the one below must be added to the code:
