...
`withClamp` is an [animation modifier](/docs/next/fundamentals/glossary#animation-modifier) that lets you limit the scope of movement of your animation to make it stay within some predefined range.
Use it with [withSpring](/docs/next/animations/withSpring) animation.
...
`withClamp` returns an [animation object](/docs/next/fundamentals/glossary#animation-object). It can be either assigned directly to a [shared value](/docs/next/fundamentals/glossary#shared-value) or can be used as a value for a style object returned from [useAnimatedStyle](/docs/next/core/useAnimatedStyle).
