...
`withDelay` is an [animation modifier](/docs/next/fundamentals/glossary#animation-modifier) that lets you start an animation with a delay.
...
`withDelay` returns an [animation object](/docs/next/fundamentals/glossary#animation-object) which holds the current state of the animation. It can be either assigned directly to a [shared value](/docs/next/fundamentals/glossary#shared-value) or can be used as a value for a style object returned from [useAnimatedStyle](/docs/next/core/useAnimatedStyle).
