...
`useAnimatedRef` lets you get a reference of a view. Used alongside [`measure`](/docs/next/advanced/measure), [`scrollTo`](/docs/next/scroll/scrollTo), and [`useScrollViewOffset`](/docs/next/scroll/useScrollViewOffset) functions.
...
- The value stored in the `current` property isn't available on the [UI thread](/docs/next/fundamentals/glossary#ui-thread).
