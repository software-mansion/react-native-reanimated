...
Data stored in shared values can be accessed and modified by their `.value` property.

#### React Compiler support

Reanimated supports alternative syntax for `useSharedValue` that is compliant with the [React Compiler](https://react.dev/learn/react-compiler) standards. You can access and modify `value` property with the use of `get` and `set` methods.
...
- Don't read or modify the `sv.value` during a component's render. Access to `.value` is a side effect, and triggering side-effects during render [violates the Rules of React](https://react.dev/reference/rules/components-and-hooks-must-be-pure#side-effects-must-run-outside-of-render). All reads from and writes to `.value` should happen in relevant callbacks, i.e. in `useAnimatedStyle` or `useEffect` hooks.
