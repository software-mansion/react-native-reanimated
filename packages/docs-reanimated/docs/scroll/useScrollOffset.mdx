...
`useScrollOffset` lets you to create animations based on the offset of a `ScrollView`.
...
`useScrollOffset` returns a [shared value](/docs/fundamentals/glossary#shared-value) which holds the current offset of the `ScrollView`.
