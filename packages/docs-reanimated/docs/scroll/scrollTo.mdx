...
An [animated ref](/docs/next/core/useAnimatedRef#returns) connected to the ScrollView (or other scrollable) component you'd want to scroll on. The animated ref has to be passed either to an [Animated component](/docs/next/fundamentals/glossary#animated-component) or a React Native built-in component.
...
- The `scrollTo` function can only be called from [the UI thread](/docs/next/fundamentals/glossary#ui-thread).
- Supports `Animated.FlatList`.
- Usually works with other ScrollView-like and FlatList-like components if they use a `ScrollView` under the hood and are [made animated](/docs/next/core/createAnimatedComponent#reference).
