...
- In browsers there's no separate UI thread available. Because of that, on the Web, `runOnUI` behaves just like `requestAnimationFrame`. It creates a function that, when called, will be scheduled to run with given arguments on next animation frame.
