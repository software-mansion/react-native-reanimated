...
In short, the Reanimated babel plugin automatically converts special JavaScript functions (called [worklets](/docs/next/fundamentals/glossary#worklet)) to allow them to be passed and run on the UI thread.
...
To learn more about the plugin head onto to [Reanimated babel plugin](/docs/next/fundamentals/glossary#reanimated-babel-plugin) section.
