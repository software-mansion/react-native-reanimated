...
`react-native-worklets` is currently a required dependency. It was separated from `react-native-reanimated` for better modularity and will become optional in the future, only being required when using worklet-based animations.
