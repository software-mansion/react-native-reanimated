...
With Reanimated, you can easily create smooth animations and interactions that run on the [UI thread](/docs/next/fundamentals/glossary#ui-thread).
...
Alternatively, you can dive into [our examples](https://github.com/software-mansion/react-native-reanimated/tree/3.17.2/apps/common-app/src/examples) on GitHub.
...
Install `react-native-reanimated@next` and `react-native-worklets@next` packages from npm:

:::note

`react-native-worklets` is required for worklet-based animations. It was separated from react-native-reanimated to allow for more modular architecture. It will eventually become optional, but is currently a required dependency.
...
<Tabs groupId="package-managers">
  <TabItem value="npm" label="NPM">
    ...
    npm install react-native-reanimated@next react-native-worklets@next

  </TabItem>
  <TabItem value="yarn" label="YARN">

    yarn add react-native-reanimated@next react-native-worklets@next
...
In short, the Worklets babel plugin automatically converts special JavaScript functions (called [worklets](/docs/next/fundamentals/glossary#worklet)) to allow them to be passed and run on the UI thread.
...
To learn more about the plugin head onto to [Worklets babel plugin](/docs/next/fundamentals/glossary#worklets-babel-plugin) section.
