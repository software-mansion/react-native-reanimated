...
You can safely access the shared values because callbacks passed to gestures are automatically [workletized](/docs/next/fundamentals/glossary#to-workletize) for you.
...
Make sure to check the full [`withDecay` API reference](/docs/next/animations/withDecay) to get to know the rest of the configuration options.
...
In [the next section](/docs/next/fundamentals/glossary), Let's explore a glossary of terms together!
