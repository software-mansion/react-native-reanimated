...
Another way of [customizing animations](/docs/next/fundamentals/customizing-animation) in Reanimated is by using animation modifiers. Reanimated comes with three built-in modifiers: [`withRepeat`](/docs/next/animations/withRepeat), [`withSequence`](/docs/next/animations/withSequence), and [`withDelay`](/docs/next/animations/withDelay).
...
If this sounds new to you, no worries! Start by going through the basics in [Your First Animation](/docs/next/fundamentals/your-first-animation) and come back here when you're ready.
...
To implement our desired shake animation we can use [`withRepeat`](/docs/next/animations/withRepeat) modifier. This modifier lets you repeat a provided animation.
...
To learn more about it you can check out the full [`withRepeat` API reference](/docs/next/animations/withRepeat).
...
To learn more about it you can check out the full [`withSequence` API reference](/docs/next/animations/withSequence).
...
To learn more about it you can check out the full [`withDelay` API reference](/docs/next/animations/withDelay).
...
In the [next section](/docs/next/fundamentals/handling-gestures), we're going to learn about handling `Tap` and `Pan` gestures. Also, we'll get to know the `withDecay` animation function.
