...
[Previous section](/docs/next/fundamentals/animating-styles-and-props) not only taught you how to use shared values in practice but also you used `withSpring` and `withTiming` functions to create animations. We think now you're more than ready to dive deeper into customizing animations!

Reanimated comes with three built-in animation functions: [`withTiming`](/docs/next/animations/withTiming), [`withSpring`](/docs/next/animations/withSpring), and [`withDecay`](/docs/next/animations/withDecay). For now, let's focus on the first two, and we'll come back to `withDecay` in the [Handling gestures](/docs/next/fundamentals/handling-gestures) section.
...
Reanimated comes with a handful of predefined easing functions. You can play around with them in the interactive playground below or check the full [`withTiming` API reference](/docs/next/animations/withTiming).
...
Reanimated comes with a handful of other properties used to customize your spring animation. You can play around with them in our interactive playground or check the full [`withSpring` API reference](/docs/next/animations/withSpring).
...
In [the upcoming section](/docs/next/fundamentals/applying-modifiers), you'll discover how to use animation modifiers such as `withSequence` and `withRepeat`. These modifiers we'll allow us to create more complex and engaging animations.
