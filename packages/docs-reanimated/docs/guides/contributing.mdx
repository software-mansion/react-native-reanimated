...
To find the native source code of Reanimated navigate to `Pods > Development Pods >  RNReanimated > Example > ... > ... > node_modules > react-native-reanimated > apple`. Making sure the Metro bundler is running. You can now build React Native app or debug native code.
