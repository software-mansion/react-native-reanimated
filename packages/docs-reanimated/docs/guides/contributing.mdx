...
Our workflow usually starts from editing `apps/common-app/src/apps/reanimated/examples/EmptyExample.tsx` with testing example for our new feature or a bug. This is the part that we often attach to PR so other developers can test the same use-case (remember not to commit your changes from `EmptyExample` file!). Next, we run the `apps/fabric-example` app and start development.
