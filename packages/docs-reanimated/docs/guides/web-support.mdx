...
Install `@babel/plugin-proposal-export-namespace-from` package from npm:
...
Add `@babel/plugin-proposal-export-namespace-from` plugin to your `babel.config.js`.
...
you need to run the following command inside the `apps/web-example` directory:
