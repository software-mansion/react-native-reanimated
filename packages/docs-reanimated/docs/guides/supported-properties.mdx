...
On the Web, to animate `textShadow` props (`textShadowColor`, `textShadowOffset`, `textShadowRadius`) using CSS Animations you need to specify them in keyframes. Consider this example:
...
<Animated.Text
  style={{
    ...
    animationName: {
      ...
      to: { textShadowColor: 'blue' },
    },
    textShadowOffset: { width: 10, height: 10 }, // 🚨 needs to be specified
    textShadowRadius: 8, // 🚨 in keyframes to work on the web
    // highlight-end
    // other styles...
  }}>
  Reanimated
...
This animation will correctly work on both iOS and Android. However, on the Web, while `textShadowColor` will animate correctly from `"red"` to `"blue"`, `textShadowOffset` and `textShadowRadius` won't be included in the keyframe, resulting in no animation.

To mitigate the problem, move `textShadow` styles to keyframes:
