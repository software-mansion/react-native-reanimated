...
**Solution:** Install `react-native-worklets` with your package manager, i.e. `yarn add react-native-worklets` or `npm i react-native-worklets`. Make sure to install a compatible version TODO:
