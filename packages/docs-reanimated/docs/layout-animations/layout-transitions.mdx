...
Just as in withSpring config, spring-based modifiers can be physics-based (has `mass` and `damping` modifiers) or duration-based (has `duration` and `dampingRatio` modifiers). Usage of any time-based spring modifiers erases the effect of any physics-based spring modifier. Defaults to physics-based.
...
- Just as in [withSpring](/docs/animations/withSpring/) config, spring-based modifiers can be physics-based (has `mass` and `damping` modifiers) or duration-based (has `duration` and `dampingRatio` modifiers). You need to decide whether you want to use physics or duration based animation.
