...
std::string_view displayName(DisplayType type) {
  switch (type) {
...
MountedNode mountedNode(const StubView &view, bool isRoot = false) {
  ...
  return {
      ...
      .opacity = opacity,
      .display = std::string(displayName(view.layoutMetrics.displayType)),
      .zIndex = zIndex,
...
folly::dynamic nodeJson(const MountedNode &node) {
  ...
  return folly::dynamic::object("tag", node.tag)("component", node.component)(
      ...
      folly::dynamic::object("x", node.frame.x)("y", node.frame.y)("width", node.frame.width)(
          "height", node.frame.height))("opacity", node.opacity)("display", node.display)("zIndex", node.zIndex)(
      "children", std::move(children));
}

MountedMutation::State mutationState(const ShadowView &view) {
  const auto &frame = view.layoutMetrics.frame;
  const auto &props = static_cast<const ViewProps &>(*view.props);
  return {
      .frame = {frame.origin.x, frame.origin.y, frame.size.width, frame.size.height},
...
folly::dynamic mutationStateJson(const std::optional<MountedMutation::State> &state) {
  if (!state) {
    return nullptr;
  }
  return folly::dynamic::object(
      "frame",
      folly::dynamic::object("x", state->frame.x)("y", state->frame.y)("width", state->frame.width)(
          "height", state->frame.height))("opacity", state->opacity)("zIndex", state->zIndex);
}
  ...
  for (const auto &mutation : frame.mutations) {
    mutations.push_back(
        folly::dynamic::object("type", mutation.type)("tag", mutation.tag)("parentTag", mutation.parentTag)(
            "index", mutation.index)("before", mutationStateJson(mutation.before))(
            "after", mutationStateJson(mutation.after)));
  }
  ...
  uiManager_->getShadowTreeRegistry().visit(surfaceId_, [&](const ShadowTree &shadowTree) {
    ...
    status = shadowTree.commit(
        ...
        ShadowTree::CommitOptions{
            .enableStateReconciliation = !mountSynchronously,
            .mountSynchronously = mountSynchronously,
            .source = mountSynchronously ? ShadowTree::CommitSource::Unknown : ShadowTree::CommitSource::React,
        });
  });
...
void PlatformDriver::recordFrame(const MountingTransaction &transaction) const {
  ...
  for (const auto &mutation : transaction.getMutations()) {
    ...
    auto before = std::optional<MountedMutation::State>{};
    auto after = std::optional<MountedMutation::State>{};
    if (mutation.type != ShadowViewMutation::Create && mutation.type != ShadowViewMutation::Insert) {
      before = mutationState(mutation.oldChildShadowView);
    }
    if (mutation.type != ShadowViewMutation::Delete && mutation.type != ShadowViewMutation::Remove) {
      after = mutationState(mutation.newChildShadowView);
    }
    mutations.push_back({
...
void PlatformDriver::runEffect(const ShadowViewMutation &mutation) const {
  ...
  switch (mutation.type) {
    case ShadowViewMutation::Insert:
      ...
      if (auto effects = treeBuilder_.findEffects(mutation.newChildShadowView)) {
    ...
    case ShadowViewMutation::Remove:
      ...
      if (auto effects = treeBuilder_.findEffects(mutation.oldChildShadowView)) {
