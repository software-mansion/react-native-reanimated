...
function decorateAnimation<T extends AnimationObject | StyleLayoutAnimation>(
  ...
  const colorOnStart = (
    ...
    if (isColor(value)) {
      RGBACurrent = convertToRGBA(animation.current);
      RGBAValue = convertToRGBA(value);
      if (animation.toValue) {
        RGBAToValue = convertToRGBA(animation.toValue);
      }
    }
    tab.forEach((i, index) => {
    ...
    animation.current = toRGBAString(res as ParsedColorArray);
  };
  ...
  const colorOnFrame = (
    ...
    const RGBACurrent = convertToRGBA(animation.current);
    const res: Array<number> = [];
    let finished = true;
    tab.forEach((i, index) => {
      animation[i].current = RGBACurrent[index];
