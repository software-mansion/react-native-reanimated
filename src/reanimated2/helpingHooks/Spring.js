...
const workletBody = new Worklet(function(sv, state, config) {
  'worklet';
  ...
  const isOvershooting = () => {
    if (config.overshootClamping.value && config.stiffness.value != 0) {
      return memory.prevPosition < config.toValue.value
        ? sv.value > config.toValue.value
        : sv.value < config.toValue.value;
    } else {
      return false;
    }
  ...
  const isVelocity =
    Math.abs(state.velocity.value) < config.restSpeedThreshold.value;
  const isDisplacement =
    config.stiffness.value == 0 ||
    Math.abs(config.toValue.value - sv.value) <
      config.restDisplacementThreshold.value;
