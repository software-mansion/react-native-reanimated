...
function makeShareableClonRecursiveWeb(value: unknown) {
  return value;
}
...
export const makeShareableCloneRecursive = SHOULD_BE_USE_WEB
  ? (makeShareableClonRecursiveWeb as typeof makeShareableCloneRecursiveNative)
...
function cloneArray<T extends unknown[]>(
  ...
  const clone = NativeReanimatedModule.makeShareableClone(
    NAME_ME,
    shouldPersistRemote
  ) as ShareableRef<T>;
